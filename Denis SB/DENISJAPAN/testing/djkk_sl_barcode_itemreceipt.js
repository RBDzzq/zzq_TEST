		...
		if(!isEmpty(poNumber)){
			var transactionSearch = nlapiSearchRecord("inventorydetail",null,
					...
					[
					   ...
					   new nlobjSearchColumn("quantityshiprecv","transaction",null),					
					   ...
					   new nlobjSearchColumn("formulanumeric1").setFormula("({transaction.quantity}-{transaction.quantityshiprecv})/({transaction.quantity}/{transaction.quantityuom})")
			...
			if(!isEmpty(transactionSearch)){
				...
				for(var i=0;i<transactionSearch.length;i++){
					...
					var itemName= transactionSearch[i].getValue(columnID[11]);
	                ...
	                jsonObj+='"locationId":"'+locationName+'",';
	                
	                var binName=transactionSearch[i].getText(columnID[5]);
	                jsonObj+='"binId":"'+binName+'",';
	                ...
	                // 20230223 by lj start
//	                var poInvQuantity= transactionSearch[i].getValue(columnID[9]);
	                var poInvQuantity= transactionSearch[i].getValue(columnID[16]);
	                jsonObj+='"poInvQuantity":"'+poInvQuantity+'",';
	                
	                var quantity= transactionSearch[i].getValue(columnID[12]);
	                jsonObj+='"quantity":"'+quantity+'",';
	                
	                var quantityuom= transactionSearch[i].getValue(columnID[13]);
	                jsonObj+='"quantityuom":"'+quantityuom+'",';
	                
	                var quantityshiprecv= transactionSearch[i].getValue(columnID[14]);
	                jsonObj+='"quantityshiprecv":"'+quantityshiprecv+'",';
	                
	                var poNeedQuantity= transactionSearch[i].getValue(columnID[15]);	                
//	                var poNeedQuantity= transactionSearch[i].getValue(columnID[10]);
	...
	// 「確定｣ボタンを押す
		...
		var backFlag=newCreatItemreceipt(poId,podetailArray['poDetail']);
...
function newCreatItemreceipt(poId,podetailArray) {
	try {
		...
		for (var q = 1; q < count + 1; q++) {
			itemreceiptRecord.selectLineItem('item', q);			
		...
		for (var w = 1; w < count + 1; w++) {
			itemreceiptRecord.selectLineItem('item',w);
			for (var i = 0; i < podetailArray.length; i++) {
				...
				for(var j=0;j<itemArray.length;j++){
					...
					if(itemreceiptRecord.getCurrentLineItemValue('item', 'orderline')==polineId
						...
						var subrecord2 = itemreceiptRecord.createCurrentLineItemSubrecord('item', 'inventorydetail');
						...
						for(var s=0;s<inventorydetailArray.length;s++){
							...
							var expirationdate_ = inventorydetailArray[s]['ed'];
							...
							if(!isEmpty(lotNumber)){													
		    				    var inventorynumberSearch = nlapiSearchRecord("inventorynumber",null,
					                    ...
					                    [
					                     	new nlobjSearchColumn("internalid"),//ロット番号internalid
					                     	new nlobjSearchColumn("expirationdate"),//有効期限
					                     	new nlobjSearchColumn("custitemnumber_djkk_maker_serial_number"),//DJ_メーカー製造ロット
					                     	new nlobjSearchColumn("custitemnumber_djkk_make_date"),//DJ_製造年月日
					                     	new nlobjSearchColumn("custitemnumber_djkk_shipment_date"),//DJ_出荷可能期限日
					                     	new nlobjSearchColumn("custitemnumber_djkk_warehouse_number"),//DJ_倉庫入庫番号
					                     	new nlobjSearchColumn("custitemnumber_djkk_smc_nmuber"),//DJ_SMC番号
					                     	new nlobjSearchColumn("custitemnumber_djkk_lot_memo"),//DJ_ロットメモ	
					                     	new nlobjSearchColumn("custitemnumber_djkk_control_number"),//DJ_メーカーシリアル番号
					                     	new nlobjSearchColumn("custitemnumber_djkk_lot_remark")//DJ_ロットリマーク
					                    ]
					                    );    
		    				    	var invReordId = defaultEmpty(inventorynumberSearch[0].getValue("internalid"));//ロット番号internalid
		    				    	var expirationdate = defaultEmpty(inventorynumberSearch[0].getValue("expirationdate"))
			    				    var makernum = defaultEmpty(inventorynumberSearch[0].getValue("custitemnumber_djkk_maker_serial_number"))
			    				    var madedate = defaultEmpty(inventorynumberSearch[0].getValue("custitemnumber_djkk_make_date"))
			    				    var deliveryperiod = defaultEmpty(inventorynumberSearch[0].getValue("custitemnumber_djkk_shipment_date"))
			    				    var warehouseCode = defaultEmpty(inventorynumberSearch[0].getValue("custitemnumber_djkk_warehouse_number"))
			    				    var smc = defaultEmpty(inventorynumberSearch[0].getValue("custitemnumber_djkk_smc_nmuber"))
			    				    var lotMemo =  defaultEmpty(inventorynumberSearch[0].getValue("custitemnumber_djkk_lot_memo"))
			    				    var controlNumber =  defaultEmpty(inventorynumberSearch[0].getValue("custitemnumber_djkk_control_number"))
			      				    var lotRemark =  defaultEmpty(inventorynumberSearch[0].getValue("custitemnumber_djkk_lot_remark"))
	    				    }
							//end
							//20230214 add by lj start
							if(isEmpty(expirationdate_)){
								expirationdate_ = expirationdate;
							}
							var inventorynumberReord = nlapiLoadRecord('inventorynumber',invReordId);
			    		    inventorynumberReord.setFieldValue('expirationdate',expirationdate_);//有効期限
							...
							//20230117 add by zhou start
//							subrecord2.setCurrentLineItemValue('inventoryassignment','expirationdate', "2023/02/14");
