...
//			return false;
		...
		if(percentAllBal!='100'&&percentAllBal!='0'&&!isEmpty(landedcostamountBal)&&landedcostamountBal!=0){
			alert('PO配賦の梱包代割合率の合計は100%ではありません');
//			return false;
		}
		//changed by geng add end
		
		//20221202 add by zhou CH147 
		//保存時にpo配賦保険料とpo明細配賦保険料とDJ _保険料比較検査
		var mainSubtype = 'custpage_main_sublist';//po配賦
		var lineSubtype = 'custpage_sublist';//po明細配賦
		var count=nlapiGetLineItemCount(mainSubtype);//po配賦
		var lineCount=nlapiGetLineItemCount(lineSubtype);//po明細配賦
		...
		if(poMainPremiumAmount != premiumAmount){
			alert('po配賦リストの「保険料」の価格総額は「DJ _保険料」と等しくありませんので、ご確認ください。')
			return false;
		}
		if(poLinePremiumAmount != premiumAmount){
			alert('po明細配賦リストの「保険料」の価格総額は「DJ _保険料」と等しくありませんので、ご確認ください。')
			return false;
		}
...
/**
 * po配賦保険料の変動時の計算-po明細配賦保険料の同時変更 CH147 20221202 add by zhou
 */
