...
function suitelet(request, response){
	nlapiLogExecution('debug','購買計画レポート_LS','start')
	...
	var form = '';
	if(searchType=='T'){
		form =nlapiCreateForm('購買計画レポート_LS', true);
	}else{
		form =nlapiCreateForm('購買計画レポート_LS', false);
	}
	form.setScript('customscript_djkk_cs_fc_ls_view_sc');
       ...
       if(!isEmpty(subsidiaryPar)&&subsidiaryPar!=0){
    	   subsidiaryField.setDefaultValue(subsidiaryPar);
    	    ...
    	    var soForecastItemSearch = nlapiSearchRecord("customrecord_djkk_so_forecast_ls",null,
    	    		...
    	    		[
						new nlobjSearchColumn("custrecord_djkk_so_fc_ls_item",null,"GROUP"),//20230411 CHANGED BY ZHOU 
						new nlobjSearchColumn("internalid","CUSTRECORD_DJKK_SO_FC_LS_ITEM","GROUP"),  //20230411 CHANGED BY ZHOU    	
    	    		]
    	    		);
    	    if (!isEmpty(soForecastItemSearch)) {
    			for (var itf = 0; itf < soForecastItemSearch.length; itf++) {
    				itemList.push(soForecastItemSearch[itf].getValue("internalid","CUSTRECORD_DJKK_SO_FC_LS_ITEM","GROUP"));
    			}
    		}
    	    if(!isEmpty(itemList)){
...
//			}
		...
		nlapiLogExecution('debug','dateArray',dateArray)
		var xmlString='';
	   ...
	   //TODO xmlString += '<Row></Row>';
		...
		htmlNote += '<td rowspan="2" style="position:sticky;left:'+tableCloum2+'px;z-index:2;background-color:#ffcc80;border:1px solid gray;border-left:0px;vertical-align:top;">'+ date + '</td>';
		 ...
		 xmlString += '<Cell ss:MergeAcross="2" ss:MergeDown="1" ss:StyleID="s16"><Data ss:Type="String">PrintDate: '+date+'</Data></Cell>';
		for (var wk = 0; wk < 54; wk++) {
			if(dateArray[wk][4]){
				htmlNote += '<td style="border:1px solid gray;background-color:#ff8c1a;color:#ffffff;">'+ dateArray[wk][0] + '</td>';
				xmlString += '<Cell ss:StyleID="s18"><Data ss:Type="String">'+dateArray[wk][0]+'</Data></Cell>';	
			}else{
				htmlNote += '<td style="border:1px solid gray;">'+ dateArray[wk][0] + '</td>';
				xmlString += '<Cell ss:StyleID="s17"><Data ss:Type="String">'+dateArray[wk][0]+'</Data></Cell>';	
			}
		}
		xmlString += '<Cell ss:StyleID="s17"><Data ss:Type="String">NOW</Data></Cell>';
		xmlString += '</Row>';
		...
		for (var ye = 0; ye < 54; ye++) {
			...
			xmlString += '<Cell ss:StyleID="s18"><Data ss:Type="String">'+dateArray[ye][1]+'</Data></Cell>';															
			    ...
			    if(ye==0){
					xmlString += '<Cell ss:Index="4" ss:StyleID="s17"><Data ss:Type="String">'+dateArray[ye][1]+'</Data></Cell>';		
				}else{
			    xmlString += '<Cell ss:StyleID="s17"><Data ss:Type="String">'+dateArray[ye][1]+'</Data></Cell>';	
			   }
			}
		}
		xmlString += '<Cell ss:StyleID="s17"><Data ss:Type="String"></Data></Cell>';
		xmlString += '</Row>';
		...
		xmlString += '<Row>';
		xmlString += '<Cell ss:MergeAcross="2" ss:StyleID="s10"><Data ss:Type="String">基準日： '+date+'</Data></Cell>';
		
		for (var sd = 0; sd < 54; sd++) {
			if(dateArray[sd][4]){
				htmlNote += '<td style="border:1px solid gray;background-color:#ff8c1a;color:#ffffff;">'+ dateArray[sd][2] + '</td>';	
				xmlString += '<Cell ss:StyleID="s18"><Data ss:Type="String">'+dateArray[sd][2]+'</Data></Cell>';	
			}else{
			htmlNote += '<td style="border:1px solid gray;">'+ dateArray[sd][2] + '</td>';
			xmlString += '<Cell ss:StyleID="s17"><Data ss:Type="String">'+dateArray[sd][2]+'</Data></Cell>';	
			}
		}
		xmlString += '<Cell ss:StyleID="s17"><Data ss:Type="String"></Data></Cell>';
		xmlString += '</Row>';
		...
		for (var itemc = 0; itemc < forecastSearch.length; itemc++) {
            ...
            xmlString += '<Row>';
            xmlString += '<Cell ss:MergeAcross="56" ss:StyleID="s11"><Data ss:Type="String">'+itemId+'                '+itemDescription+'</Data></Cell>';
			...
			htmlNote += '</tr>';
			xmlString += '<Cell ss:MergeDown="6" ss:StyleID="s14"><Data ss:Type="String">'+itemLocationInventory+'</Data></Cell>';
			xmlString += ' </Row>';
...
//				}
	...
	if(searchType=='T'){
	var execlUrl = "window.location.href='" + excelDown(xmlString) + "'";	
	// Excel出力ボタン
 ...
 * @returns {void} なし
 */
function excelDown(mainxmlString) {
	...
	xmlString += '<Alignment ss:Vertical="Center"/><Borders/><Font ss:FontName="'+ font + '" ss:Size="11" ss:Color="#000000"/>';
	...
	xmlString += ' </Borders><Font ss:FontName="' + font+ '" ss:Size="11" ss:Color="#000000"/>';
	...
	xmlString += ' </Borders><Font ss:FontName="' + font+ '" ss:Size="11" ss:Color="#000000"/>';
	xmlString += '<Interior ss:Color="#E7E6E6" ss:Pattern="Solid"/> </Style>';
	xmlString += '<Style ss:ID="s10"> <Alignment ss:Horizontal="Right" ss:Vertical="Bottom"/>';
	xmlString += '<Borders><Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1"/>';
	xmlString += ' <Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="1"/>';
	xmlString += '<Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="1"/>';
	xmlString += '<Border ss:Position="Top" ss:LineStyle="Continuous" ss:Weight="1"/>';
	...
	xmlString += ' </Borders><Font ss:FontName="' + font+ '" ss:Size="11" ss:Color="#000000"/>';
	xmlString += ' <Interior ss:Color="#E7E6E6" ss:Pattern="Solid"/></Style>';
	xmlString += '<Style ss:ID="s16"><Alignment ss:Horizontal="Right" ss:Vertical="Top"/>';
	xmlString += ' <Borders><Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="1"/>';
	xmlString += '<Border ss:Position="Top" ss:LineStyle="Continuous" ss:Weight="1"/>';
	xmlString += ' </Borders><Interior ss:Color="#FFCC80" ss:Pattern="Solid"/></Style>';
	xmlString += '<Style ss:ID="s17"><Borders><Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1"/>';
	xmlString += ' <Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="1"/>';
	xmlString += ' <Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="1"/>';
	xmlString += '  <Border ss:Position="Top" ss:LineStyle="Continuous" ss:Weight="1"/>';
	xmlString += ' </Borders><Interior ss:Color="#FFCC80" ss:Pattern="Solid"/></Style>';
	xmlString += ' <Style ss:ID="s18"><Borders><Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1"/>';
	xmlString += '  <Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="1"/>';
	xmlString += '  <Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="1"/>';
	xmlString += ' <Border ss:Position="Top" ss:LineStyle="Continuous" ss:Weight="1"/>';
	xmlString += ' </Borders><Interior ss:Color="#FF8C1A" ss:Pattern="Solid"/></Style>';
	xmlString += '<Style ss:ID="s19"><Borders><Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1"/>';
	xmlString += ' <Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="1"/>';
	xmlString += ' <Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="1"/>';
	xmlString += ' <Border ss:Position="Top" ss:LineStyle="Continuous" ss:Weight="1"/>';
	xmlString += '</Borders><Interior ss:Color="#E6E6E6" ss:Pattern="Solid"/></Style>';
	...
	xmlString += ' </Borders><Font ss:FontName="' + font+ '" ss:Size="11" ss:Color="#000000"/>';
	xmlString += ' <Interior ss:Color="#E7E6E6" ss:Pattern="Solid"/></Style>';
	xmlString += ' <Style ss:ID="s21"><Borders><Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1"/>';
	xmlString += ' <Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="1"/>';
	xmlString += ' <Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="1"/>';
	xmlString += ' <Border ss:Position="Top" ss:LineStyle="Continuous" ss:Weight="1"/>';
	xmlString += ' </Borders><Interior ss:Color="#E7E6E6" ss:Pattern="Solid"/></Style>';
    ...
    xmlString += '<Worksheet ss:Name="購買計画レポート_LS">';
    xmlString += '<Table ss:ExpandedColumnCount="58" ss:ExpandedRowCount="1000" x:FullColumns="1" x:FullRows="1" ss:DefaultRowHeight="14">';
    ...
    var xlsFile = nlapiCreateFile('購買計画レポート_LS' + '_' + getFormatYmdHms() + '.xls', 'EXCEL', nlapiEncrypt(xmlString, 'base64'));
    xlsFile.setFolder('791');
...
function getOutWeekDate (yearAndWeekNo, currentMonthOfYear, outCurrent, lastMonthOfYear, outLast, nextMonthOfYear, outNext) {
    ...
    nlapiLogExecution('debug','in',yearAndWeekNo)
    // result
    var res = null;
  ...
  //20230407 changed by zhou start
    ...
    if(yearAndWeekNo == '2023-22'){
    	nlapiLogExecution('debug','res',res)
    	nlapiLogExecution('debug','currentMonthOfYear',currentMonthOfYear)
    	nlapiLogExecution('debug','lastMonthOfYear',lastMonthOfYear)
    	nlapiLogExecution('debug','nextMonthOfYear',nextMonthOfYear)
    	nlapiLogExecution('debug','currentMonthAvrgOut',currentMonthAvrgOut)
    	nlapiLogExecution('debug','lastMonthAvrgOut',lastMonthAvrgOut)
