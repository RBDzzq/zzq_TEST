 ...
 */
 function suitelet(request, response){
	 nlapiLogExecution('debug','営業計画情報レポート_LS','start')
        ...
        for (var i = 0; i < itemLen; i++) {
            ...
            for (var l = 0; l < locLen; l++) {
                ...
                for (var m = 0; m < 12; m++) {
                    ...
                    var userName = userIdName[singleUserId][1] +""+ userIdName[singleUserId][0];
                    var itemName = fcAcArr[i].itemName;
                    var year = yyyyMMObj.yearArr[m];
                    var month = yyyyMMObj.monthArr[m];
                    var theBaseDate = pDate;
                    var lastYearQuantity = fcAcArr[i].locations[l].actLast[m];
                    var nowYearQuantity = fcAcArr[i].locations[l].actNow[m];
                    var fcOthersQuantity = fcAcArr[i].locations[l].fcOthersArr[m];
//                    csvDetailStr += userIdName[singleUserId][1]+","fcAcArr[i].itemName+","yyyyMMObj.yearArr[m]+","yyyyMMObj.monthArr[m]+","+new Date(baseYear, baseMonth, 1)+","+fcAcArr[i].locations[l].actLast[m]+","+fcAcArr[i].locations[l].actNow[m]+","+fcAcArr[i].locations[l].fcOthersArr[m]+","+comparation;
                    csvDetailStr += userName+","+itemName+","+year+","+month+","+theBaseDate+","+lastYearQuantity+","+nowYearQuantity+","+fcOthersQuantity+","+comparation;
                    csvDetailStr += "\r\n";
        ...
        //CSV
        csvMainString += 'DJ_BP,DJ_商品コード,DJ_年,DJ_月,基準日,(Year-1),Year,自分以外,比較／入力\r\n'+csvDetailStr; 
        /***************************************************************/
    }

    return {htmlNote : htmlNote, xmlNote : xmlNote, xmlRowCnt : xmlRowCnt, csvMainString:csvMainString};
}
