...
function scheduled(type) {
    nlapiLogExecution('debug', 'DJ_SC課FC作成定期 Start');
    // parameter: キャシューID
    var cacheId = nlapiGetContext().getSetting('SCRIPT', 'custscript_djkk_fc_cache_table_so_id_ls');
    // parameter: 日付
    var date = nlapiGetContext().getSetting('SCRIPT', 'custscript_djkk_so_date_ls');
    // parametersをlogに出力
    ...
    if(!isEmpty(cacheId) && !isEmpty(date)){
        ...
        for (var i = 0; i < jLen; i++) {
        	var employeeid=jsonData[0].employee;
