...
//		nlapiLogExecution('debug', 'purchaseorderId',purchaseorderId);
	    ...
	    //add by zzq CH641 20230613 start
...
//        }
        ...
        var vendorSearchBill = nlapiSearchRecord("vendor",null,
                ...
                [
                 ...
                 // add by CH641 zzq 20230609 start
                 new nlobjSearchColumn("state","billingAddress",null), //請求先住所 : 都道府県
                 // add by CH641 zzq 20230609 end
                 // add by CH733 zzq 20230720 start
                 new nlobjSearchColumn("billaddress")
                 // add by CH733 zzq 20230720 end
                ]
                );
...
//        if (vendorSearchBill) {
            ...
            // add by CH641 zzq 20230609 start
            var billstateEn = defaultEmpty(isEmpty(vendorSearchBill) ? '' :  transfer(vendorSearchBill[0].getValue("state","billingAddress",null)));
            // add by CH733 zzq 20230720 start
            var billcountr = defaultEmpty(isEmpty(vendorSearchBill) ? '' :  vendorSearchBill[0].getValue("country","billingAddress",null));
            var billcountryEn = '';
            if(billcountr){
                if(billcountr == 'JP'){
                    billcountryEn = defaultEmpty(isEmpty(vendorSearchBill) ? '' :  transfer(vendorSearchBill[0].getText("country","billingAddress",null)));
                }else{
                    billcountryEn = defaultEmpty(isEmpty(vendorSearchBill) ? '' :  transfer(vendorSearchBill[0].getValue("billaddress")));
                    if(billcountryEn){
                        billcountryEnArr = billcountryEn.split('\n');
                        billcountryEn = billcountryEnArr[billcountryEnArr.length-1];
                    }
                }
            }
            // add by CH733 zzq 20230720 end
            var billEnAddress = billcity+ '&nbsp;' + billstateEn+ '&nbsp;' + billzipcode+ '&nbsp;' + billcountryEn;
            // add by CH641 zzq 20230609 end
//        }
            
            billCustrecord_djkk_address_fax = defaultEmpty(vendorRecord.getFieldValue('fax'));
            if(!isEmpty(billCustrecord_djkk_address_fax)){
            	billCustrecord_djkk_address_fax = billCustrecord_djkk_address_fax.slice(3)
            }
            billphone = defaultEmpty(vendorRecord.getFieldValue('phone'));
            if(!isEmpty(billphone)){
            	billphone = billphone.slice(3)
            }
		...
		var subsidiarySearch= nlapiSearchRecord("subsidiary",null,
				...
				[
				   ...
				   //CH655 20230728 add by zdj start
                   new nlobjSearchColumn("phone","returnaddress",null),//配送先phone
                   new nlobjSearchColumn("custrecord_djkk_address_fax","returnaddress",null)//配送先fax
                   //CH655 20230728 add by zdj end
				]
	);
		...
		//CH655 20230728 add by zdj start
        var returnaddressPhone= defaultEmpty(isEmpty(subsidiarySearch) ? '' :  subsidiarySearch[0].getValue("phone","returnaddress",null));//返品先phone
        var returnaddressFax= defaultEmpty(isEmpty(subsidiarySearch) ? '' :  subsidiarySearch[0].getValue("custrecord_djkk_address_fax","returnaddress",null));//返品先fax
        //CH655 20230728 add by zdj end
...
//	    nlapiLogExecution('debug', 'incotermSiteValue', incotermSiteValue);
		...
		if(itemList != 0) {
	      for(var s = 1; s <= itemList; s++){
	    	     ...
	    	     // add by zzq CH733 20230707 start
	    	  	var itemType = defaultEmpty(record.getLineItemValue('item', 'itemtype', s));//アイテム種類
	    	     // add by zzq CH733 20230707 end
...
//	    	  	var quantityFormat = defaultEmptyToZero(quantity.toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,'));
				...
				// CH591　zheng 20230601 start
				// if(!isEmpty(language)&&!isEmpty(units_display)&& customForm == 169){
			    if(!isEmpty(language)&&!isEmpty(units_display)){
			    // CH591　zheng 20230601 end
...
//						}else if(language == '26'){				//日本語
						}else if(language == LANGUAGE_JP){				//日本語
						  //add by zzq CH641 20230613 end
							units_display = unitSearch[0].getValue('abbreviation')+'';
							unitsArray = units_display.split("/");
...
//						if(language == '13'){			//英語
					    ...
					    if (itemType == 'otherchargeitem') {   //アイテムは再販用その他の手数料
					    	var itemName2 = '';
					    	itemName1 = othChargeDisplayname(itemName, language);  //手数料
					    }else {
					    	if (language == LANGUAGE_EN) { // 英語
					    		// add by zzq CH641 20230613 end
					...
					// by add zzq CH671 20230704 end
						...
						for(var j = 0; j < itemDetails.length; j++){
							if ( itemDetails[j].item == itemId) {
								...
								nlapiLogExecution('error', 'JGitemName1', itemName1);
								itemDetails[j].itemName2 = itemName2;
								nlapiLogExecution('error', 'JGitemName2', itemName2);
			...
			//CH655 20230727 add by zdj start
			var departmentPhone = '';
	        var departmentFax = '';
	        
	        var departmentId = record.getFieldValue('department');      //セクションPhnoeとFax取得　01
	        ...
	        var departmentSearch = getSearchResults("department",null,
	                ...
	                [
	                   new nlobjSearchColumn("custrecord_djkk_dp_phone"),
	                   new nlobjSearchColumn("custrecord_djkk_dp_fax")
	                ]
	                );
	        
	        departmentPhone = defaultEmpty(isEmpty(departmentSearch) ? '' : transfer(departmentSearch[0].getValue("custrecord_djkk_dp_phone")));
	        departmentFax = defaultEmpty(isEmpty(departmentSearch) ? '' : transfer(departmentSearch[0].getValue("custrecord_djkk_dp_fax")));
	        }
	        //CH655 20230727 add by zdj start
...
//	    var subsidiaryName
		...
		//add by zzq バグ対応　20230620 start
		 // 英文仕入先名
		if(language == LANGUAGE_EN){
	        companyname = transfer(defaultEmpty(vendorRecord.getFieldValue('custentity_djkk_name_english')));;
	    }
		//add by zzq バグ対応　20230620 end
	    // 18日付(発注日の認識)
...
//	    var poNumber = defaultEmpty(record.getFieldValue('custbody_djkk_production_po_number')) ;
	    ...
	    // 21支払条件 3.18
	    var paymentTerms = '';
	    // CH592 zdj 2023/7/25 start
		// var paymentTermsAll =
		// transfer(defaultEmpty(record.getFieldText('terms')));
		// var paymentTermsArray=paymentTermsAll.split("/");
		var paymentTermsArray;
		var paymentTermsAll = transfer(defaultEmpty(vendorRecord
				.getFieldText('terms')));

		var paymentTermsLine = transfer(defaultEmpty(vendorRecord
				.getLineItemValue('recmachcustrecord_suitel10n_jp_pt_vendor',
						'custrecord_jp_payterm_description', 1)));

		if (paymentTermsAll) {
			paymentTermsArray = paymentTermsAll.split("/");
		} else if (!paymentTermsAll && paymentTermsLine) {
			paymentTermsArray = paymentTermsLine.split("/");
		} else if (!paymentTermsAll && !paymentTermsLine) {
			paymentTermsArray = [];
		}
		// CH592 zdj 2023/7/25 end
		...
		if(subsidiary == SUB_DPKK || subsidiary == SUB_SCETI){
		    FixedPhone = departmentPhone;
		    FixedFax = departmentFax;
		//CH655 20230728 add by zdj start
		}else if(subsidiary == SUB_NBKK || subsidiary == SUB_ULKK){
		    FixedPhone = returnaddressPhone;
		    FixedFax = returnaddressFax;
		}
		//CH655 20230728 add by zdj end
		//CH655 20230727 add by zdj end
...
//	    if (DjLanguage == '英語') {
	    if (language == LANGUAGE_EN) {
	      //add by zzq CH641 20230613 end
		    ...
		    }else if(paymentTermsArray.length == 0){
		    	paymentTerms = '';
		    }
		    // billToName設定
...
//		    var FixedNameTotal = 'total';
		    ...
		    var FixedNameOrderTitle = '下記の通り注文致します';
		    var FixedNameLaTest = '最新';
		    var FixedNameOrderConfirm1 = 'ご捺印の上ご返送ください';
		    var FixedNameOrderConfirm2 = ' ';
		    var FixedNameReceptionDate = '受付日:'; 
		    var FixedNameAuthorizedSignautre = 'ご署名:'; 
...
//	'<td align="left" style="width:30%; padding: 0em 0em 0.3em 1.3em;"><img src="' +subsidiaryLogoURL+ '" style=" height: 30px; margin-top: 0px; float: left;" /></td>'+
	'<td align="left" style="width:30%; padding: 0em 0em 0.3em 1.3em;"><img src="' +subsidiaryLogoURL+ '" style=" width: 140px; height: 18px; margin-top: 0px; float: left;" /></td>'+
	//20230731 CH756&CH757 by zdj end
	'<td align="right" style="padding: 0em 0em 0em 0em;font-size:16pt;" valign="middle"><span style="font-family: heiseimin;">'+ legalname +'</span></td>'+
	'</tr></table>'+
	...
	'</tr>'+
	// add by CH641 zzq 20230615 start
//	'<tr>'+
	'<tr>';
	if (language == LANGUAGE_JP){
	    str += '<td style="height: 17px;">&nbsp;&nbsp;&nbsp;&nbsp;'  + billzipcode + '&nbsp;' + billstate + billcity  + '</td>'+
//	    '<td style="padding-left:10px;height: 17px;">'+ FixedNameTo + billattention + '</td>'+
	    '<td colspan="2" style="padding-left:10px;height: 17px;">'+ FixedNameTo + billattention + '</td>'+
	    '</tr>'+
	    '<tr>'+
        ...
        str += '<td style="height: 17px;">&nbsp;&nbsp;&nbsp;&nbsp;' + billaddress + '</td>'+
//        '<td style="padding-left:10px;height: 17px;">'+ FixedNameTo + billattention + '</td>'+
        '<td colspan="2" style="padding-left:10px;height: 17px;">'+ FixedNameTo + billattention + '</td>'+
        '</tr>'+
        '<tr>'+
        '<td style="padding-left:20px;height: 17px;">&nbsp;&nbsp;&nbsp;&nbsp;'+ billEnAddress +'</td>'+
        '<td style="padding-left:10px;height: 17px;">'+ FixedNameTel + String(billphone) + '</td>'+
        '</tr>'+
        '<tr>'+
        '<td style="padding-left:20px;height: 17px"></td>'+
...
//	if (language == LANGUAGE_JP){
//	  //add by zzq CH641 20230613 end
//	    str +='<td style="padding-left:20px;height: 17px"></td>';
//	}else{
//	    str +='<td style="padding-left:20px;height: 17px">&nbsp;&nbsp;&nbsp;&nbsp;'+ billEnAddress +'</td>';
//	}
//	str +='<td style="padding-left:10px;height: 17px;">'+ FixedNameFax + billCustrecord_djkk_address_fax +'</td>'+
	// add by CH641 zzq 20230609 end
//	'<td align="right" style="width: 220px;height: 17px;">' + FixedNamePage + ' &nbsp; <pagenumber/>&nbsp;/&nbsp;<totalpages/>&nbsp;</td>'+
	str += '<td align="right" style="width: 220px;height: 17px;">' + FixedNamePage + ' &nbsp; <pagenumber/>&nbsp;/&nbsp;<totalpages/>&nbsp;</td>'+
	// add by CH641 zzq 20230615 end
	'</tr>'+
	'</table>'+
	'&nbsp;'+
	''+
...
//		str +='<td colspan="2" style="height: 17px;">'+ shipAddress1 + shipAddress2 + shipAddress3+'</td>'
		str +='<td colspan="2" style="height: 17px;">'+ shipAddress1 +'</td>'
	}
	/**********new*********/
	//20230314 changed by zhou end
	// DENISJAPANDEV-1383 zheng 20230303 start
...
//		str +='<td colspan="2" style="height: 17px;">'+ FixedNameAttn + shipAttention + '</td>';
		str +='<td colspan="2" style="height: 17px;">'+ shipAddress2 + '</td>';
	}
	//20230314 changed by zhou end
...
//			str +='<td colspan="2" style="height: 17px;">Tel: ' + shipAddressphone + '</td>';
			str +='<td colspan="2" style="height: 17px;">' + shipAddress3 + '</td>';
		}
		//20230314 changed by zhou end
		var tmpPersons = '';
...
//		if (DjLanguage == '英語') {
		if (language == LANGUAGE_EN) {
		    ...
		    if (salesrepresentativeCh) {
		        if (tmpEmployeeNames) {
		            tmpPersons = defaultEmpty(salesrepresentativeCh.lastname) +'&nbsp;'+ defaultEmpty(salesrepresentativeCh.firstname)  + comma;
		        } else {
		            tmpPersons = defaultEmpty(salesrepresentativeCh.lastname) +'&nbsp;'+ defaultEmpty(salesrepresentativeCh.firstname);
		        }
		    }
		}
		// CH591　zheng 20230601 start
...
//		if (DjLanguage == '英語') {
		if (language == LANGUAGE_EN) {
		  //add by zzq CH641 20230613 end
		    ...
		    str4  = '<td style="height: 17px;">'+ FixedNamePersonInCharge  + tmpPersons + employeelastname +'&nbsp;'+ employeefirstname +'</td>'   
		}
		// CH591　zheng 20230601 end
		 //if (DjLanguage == '日本語'){
		 //}else{
...
//		'<td style="width: 300px;height: 17px;">'+ FixedNamePersonInCharge  + salesrepresentative + comma + +employeefirstname +'&nbsp;'+ employeelastname +'</td>'+
		...
		'<tr>';
		if(!isEmpty(destinationCustoms)){
		    str +='<td colspan="3" style="height: 17px;"></td>';
		}else{
		    str +='<td style="height: 17px;"></td>'+
		    '<td colspan="2" style="height: 17px;">'+ FixedNameAttn + shipAttention + '</td>';
		}
		//CH655 20230727 add by zdj start
//		str5  = '<td style="height: 17px;">'+ FixedNameTel + subsidiaryTel + '</td>'
		str5  = '<td style="height: 17px;">'+ FixedNameTel + FixedPhone + '</td>'
		//CH655 20230727 add by zdj end
		//add by zzq CH641 20230613 start
//		 if (DjLanguage == '日本語'){
		 if (language == LANGUAGE_JP){
...
//		'<td style="height: 17px;">'+ FixedNameTel + employeePhone + '</td>'+
		...
		str += '</tr>'+
	    // add by zzq CH715 20230705 start
		'<tr>';
//		'<td colspan="3" style="height: 17px;"></td>'
		if(!isEmpty(destinationCustoms)){
            str += '<td colspan="3" style="height: 17px;"></td>';
        ...
        // add by zzq CH715 20230705 end
		//CH655 20230727 add by zdj start
//		str6  = '<td style="height: 17px;">'+ FixedNameFax + subsidiaryFax + '</td>'
		str6  = '<td style="height: 17px;">'+ FixedNameFax + FixedFax + '</td>'
		//CH655 20230727 add by zdj end
		//add by zzq CH641 20230613 start
//		 if (DjLanguage == '日本語'){
		 if (language == LANGUAGE_JP){
...
//	'<td align="right" style="width: 256px;"><img src="/core/media/media.nl?id=15969&amp;'+URL_PARAMETERS_C+'&amp;h=xwGkaOObH6n1hx7iEIKK7IzXqcP3XDaiz3GzyhnaY1td5xCX" style="width: 90px; height: 50px; margin-top: 5px; float: right;" /></td>'+
	'<td align="right" style="width: 256px;">&nbsp;</td>'+
	//20230731 CH756&CH757 by zdj end  
	'</tr></table>'+
	'</macro>'+
	'</macrolist>'+
	...
	'</style>'+
	'</head>'+
	'<body header="nlheader" header-height="43.3%" footer="nlfooter" footer-height="3.3%" padding="0.2in 0.5in 0.5in 0.5in" size="A4">'+
...
//	'</tr>';
		...
		for(var j =0; j < itemDetails.length;j++){
		    ...
		    if (itemDetails[j].itemType != 'discountitem') {
		        if (itemDetails[j].units_display) {
	                displayQuantity = itemDetails[j].quantity + '&nbsp;' + itemDetails[j].units_display + '&nbsp;';		            
		        } else {
		            displayQuantity = itemDetails[j].quantity + '&nbsp;';
		        }
		...
		//		var itemDetail = itemDetails[j];
				...
				if(itemDetails[j].itemType != 'subtotalitem'&&!isEmpty(itemDetails[j].itemType)){
					...
					if (tmpDetailCnt % 8 == 0) {
					    str += '<td style="width: 25px;">'+ prefixInteger(tmpDetailCnt, 2) +'0</td>'+
		                    ...
		                    '<td>&nbsp;'+ com_escapeXML(getStrLenSlice(itemDetails[j].itemName1, 21))  +'</td>'+
		                    '</tr>'+
		                    '<tr>'+
		                    ...
		                    for(var dcp = 0 ; dcp < descriptionArr.length ; dcp++){
		                        ...
		                        str +='<tr>'+
		                        '<td>&nbsp;'+com_escapeXML(getStrLenSlice(descriptionArr[dcp], 21))+'</td>'+
		                        '</tr>';
		                    }
		                    str +='</table>'+
		                    '</td>'+
					    ...
					    str += '<td style="width: 25px;">'+ prefixInteger(tmpDetailCnt, 2) +'0</td>'+
		                    ...
		                    '<tr>'+
		                    // '<td>'+ itemDetails[j].itemName2  +'</td>'+
		                    '<td>&nbsp;'+ com_escapeXML(getStrLenSlice(itemDetails[j].itemName2, 21))  +'</td>'+
		                    '</tr>'+
		                    // DENISJAPANDEV-1383 zheng 20230301 end
		                    '<tr>'+
		                    '<td>';
...
//				var itemDetail = itemDetails[j];
                    ...
                    for(var dcp = 0 ; dcp < descriptionArr.length ; dcp++){
                        ...
                        str +='<tr>'+
                        '<td>&nbsp;'+com_escapeXML(getStrLenSlice(descriptionArr[dcp], 20))+'</td>'+
                        '</tr>';
                    }
                    str +='</table>'+
                    '</td>'+
...
//	    if (DjLanguage == '日本語') {
	...
	//  '<td height="20px" align="left" colspan="4">'+vendorPrecautions+'</td>'+//todo
	    '</tr>';
	       ...
	       if (cautiondescTitle) {
	           cautiondescTitle = com_escapeXML(cautiondescTitle);
	           str+='<tr>'+
	            '<td height="20px" >&nbsp;</td>'+
...
//	           cautiondescDetail = com_escapeXML(cautiondescDetail);
	           cautiondescDetail = cautiondescDetail;
...
//	               var tmpValue = getStrLenSlice(tmpCautiondescDetails[pt], 39);
	               var tmpValue = getStrLenSlice(tmpCautiondescDetails[pt], 38);
	           ...
	           str+='<tr>'+
				...
				// '</tr>';
				'<td height="50px">&nbsp;</td>'
				+ '<td height="68px" style="background-color:#DADADA;" colspan="3">'
				+ com_escapeXML(cautiondescDetail) + '</td>' + '</tr>';
				str += '<tr>' + 
				'<td height="5px">&nbsp;</td>' +
				'</tr>';
				// CH592 zdj 2023/7/25 end
...
//    var xlsFileo = nlapiCreateFile('発注書PDF' + '_' + getFormatYmdHms() + '.csv', 'CSV', str);
...
//    nlapiSubmitFile(xlsFileo);
    ...
    var SAVE_PDF_FOLDER = PURCHASEORDER_PDF_DJ_PURCHASEORDERPDF;
    if (subsidiary == SUB_NBKK) {
        SAVE_PDF_FOLDER = PURCHASEORDER_PDF_DJ_PURCHASEORDERPDF_NBKK;
    } else if(subsidiary == SUB_ULKK){
        SAVE_PDF_FOLDER = PURCHASEORDER_PDF_DJ_PURCHASEORDERPDF_ULKK;
    } 
...
//	xlsFile.setName('PDF' + '_' + getFormatYmdHms() + '.pdf');
	xlsFile.setName('発注書' + '_' + poNumber + '_' + getDateYymmddFileName() + '.pdf');
...
function othChargeDisplayname(dpn,language) {
    ...
    if (dpn) {
        itemName1 = dpn.split("/");
            if (language == LANGUAGE_JP) {     //日本語
                itemName1 = itemName1[0];
            }else if (language == LANGUAGE_EN && itemName1.length > 1) {  //英語
                itemName1 = itemName1[1];
            }else{
            	 itemName1 = itemName1[0];
            }
        }
...
// CH591 zheng 20230602 start
function com_escapeXML(value) {
    var reValue = '';
