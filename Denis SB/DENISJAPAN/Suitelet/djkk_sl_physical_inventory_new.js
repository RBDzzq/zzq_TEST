...
function suitelet(request, response) {
    ...
    var div = nlapiGetContext().getSetting('SCRIPT', 'custscript_djkk_sl_phy_inv_div_new');
...
function run(request, response, div) {
    ...
    for (var i = 0; i < theCount; i++) {
        ...
        if (div == '1') {
            str += item_id + "_" + inv_no + "_" + location_id + "_" + binnumber_id + "_" + vo_or_cu_id + "_" + count + "_" + count_real + "_" + averagecost + "_" + expirationdate + "_" + item_brand + "_" + item_product_code + "_" + item_displayname + "_" + item_remark + "_" + item_name_english + "_" + item_vendorname + "_" + item_perunitquantity + "_" + item_memo + "_" + location + ",";
        } else {
            str += item_id + "_" + inv_no + "_" + location_id + "_" + binnumber_id + "_" + vo_or_cu_id + "_" + count + "_" + count_real + "_" + averagecost + "_" + expirationdate + "_" + item_memo_real + "_" + inventory_binnumber + "_" + binnumber + ",";
        }
...
function createForm(request, response, div) {
    ...
    // 倉庫設定
    var locationSearch = nlapiSearchRecord("location", null, locSelect, [new nlobjSearchColumn("name").setSort(false), new nlobjSearchColumn("internalid")]);
    ...
    if (!isEmpty(subValue)) {
        ...
        if (!isEmpty(locationValue)) {
            ...
            if (isEmpty(parent)) {
                ...
                var LocationList = getSearchResults("location", null, ["name", "contains", locationName], [new nlobjSearchColumn("internalid")]);
    ...
    var locationRst = nlapiSearchRecord("location", null, select, [new nlobjSearchColumn("internalid")]);
    ...
    if (selectFlg == 'T') {
        ...
        if (div == '1') {
            ...
            // 基本単位転換
            var unitstypeSearch = getSearchResults("unitstype", null, [["isinactive", "is", "F"]], [new nlobjSearchColumn("internalid"), new nlobjSearchColumn("unitname"), new nlobjSearchColumn("conversionrate"), new nlobjSearchColumn("name")]);
            ...
            [new nlobjSearchColumn("internalid"), new nlobjSearchColumn("itemid"), new nlobjSearchColumn("displayname"), new nlobjSearchColumn("salesdescription"), new nlobjSearchColumn("quantityonhand", "inventoryNumber", null), new nlobjSearchColumn("inventorynumber", "inventoryNumber", null), new nlobjSearchColumn("internalid", "inventoryNumber", null).setSort(false), new nlobjSearchColumn("location", "inventoryNumber", null), new nlobjSearchColumn("stockunit"),
                    new nlobjSearchColumn("unitstype"), new nlobjSearchColumn("averagecost"), new nlobjSearchColumn("expirationdate", "inventoryNumber"),
                    // changed by geng add start U705
                    new nlobjSearchColumn("custitem_djkk_class"), new nlobjSearchColumn("custitem_djkk_product_code"), new nlobjSearchColumn("custitem_djkk_product_name_line1"), new nlobjSearchColumn("custitem_djkk_product_name_line2"), new nlobjSearchColumn("vendorname"), new nlobjSearchColumn("custitem_djkk_perunitquantity"), new nlobjSearchColumn("custitemnumber_djkk_lot_remark", "inventoryNumber", null), new nlobjSearchColumn("vendor"),
                    // changed by geng add end U705
                    new nlobjSearchColumn("stockunit"), new nlobjSearchColumn("custitemnumber_djkk_warehouse_number", "inventoryNumber", null), new nlobjSearchColumn("custitemnumber_djkk_maker_serial_number", "inventoryNumber", null)

            ]);

            var search_2 = getSearchResults("inventorydetail", null, [["inventorynumber.quantityavailable", "greaterthan", "0"], "AND", ["binnumber", "noneof", "@NONE@"]

            ], [new nlobjSearchColumn("internalid", null, "MAX"), new nlobjSearchColumn("internalid", "inventoryNumber", "GROUP"), new nlobjSearchColumn("internalid", "binNumber", "MAX"), new nlobjSearchColumn("binnumber", "binNumber", "MAX")

            ]);
            var search_2_inventoryNumber_id = new Array();
            if (!isEmpty(search_2)) {
                for (var i = 0; i < search_2.length; i++) {
                    search_2_inventoryNumber_id.push(search_2[i].getValue("internalid", "inventoryNumber", "GROUP"));
            ...
            if (!isEmpty(search_1)) {
                ...
                for (var i = 0; i < search_1.length; i++) {
                    ...
                    if (search_2_inventoryNumber_id_index < 0) {
                        ...
                        binnumber_id = search_2[search_2_inventoryNumber_id_index].getValue("internalid", "binNumber", "MAX");
                        binnumber = search_2[search_2_inventoryNumber_id_index].getValue("binnumber", "binNumber", "MAX");
                    }
                    // 管理番号
                    ...
                    strCsv += '"' + item_id + '","' + item_name + '","' + item_english + '","' + item_product_code + '","' + item_vendorname + '","' + item_perunitquantity + '","' + location_id + '","' + location + '","' + binnumber_id + '","' + binnumber + '","' + inv_no_id + '","' + inv_no + '","' + invWarehouseNumber + '","' + invAkerSerialNumber + '","' + expirationdate + '","' + item_remark + '","' + averagecost + '","' + count + '","' + invStockUnit + '","' + vo_or_cu_id + '","'
                            + vo_or_cu + '","' + item_brand + '","' + count_real + '"';
                    strCsv += '\r\n';
            ...
            // changed by geng add start U705
            // xmlString += 'アイテムID,アイテム名,アイテム説明,管理番号（シリアル/ロット番号）ID,管理番号（シリアル/ロット番号）,場所ID,場所,ブランド,カタログコード ,商品名 ,ロットリマーク
            // ,商品名（英語）,仕入先商品コード,入り数,保管棚ID,保管棚,仕入先ID,仕入先,Total Stock Value/金額 ,賞味期限,在庫数量,実地数量\r\n'+strCsv;
            var csvTitleList = [];
            ...
            // changed by geng add end U705
            // xmlString += 'アイテムID,アイテム名,アイテム説明,管理番号（シリアル/ロット番号）ID,管理番号（シリアル/ロット番号）,場所ID,場所,保管棚ID,保管棚,仕入先ID,仕入先,金額
            // ,賞味期限,在庫数量,実地数量\r\n'+strCsv;
            ...
            var search_1 = getSearchResults("customrecord_djkk_inventory_in_custody_l", null, [["custrecord_djkk_icl_quantity_inventory", "greaterthanorequalto", "0"], "AND", ["custrecord_djkk_icl_inventorydetails", "isnotempty", ""], "AND", ["custrecord_djkk_icl_inventory_in_custody.custrecord_djkk_ic_subsidiary", "anyof", subValue]], [new nlobjSearchColumn("custrecord_djkk_icl_item"), new nlobjSearchColumn("salesdescription", "CUSTRECORD_DJKK_ICL_ITEM", null),
                    new nlobjSearchColumn("custrecord_djkk_ic_customer", "CUSTRECORD_DJKK_ICL_INVENTORY_IN_CUSTODY", null), new nlobjSearchColumn("custrecord_djkk_ic_subsidiary", "CUSTRECORD_DJKK_ICL_INVENTORY_IN_CUSTODY", null), new nlobjSearchColumn("internalid"), new nlobjSearchColumn("custrecord_djkk_icl_inventorydetail_link"), new nlobjSearchColumn("custrecord_djkk_icl_inventorydetails"), new nlobjSearchColumn("custrecord_djkk_icl_cuslocation"),
                    new nlobjSearchColumn("internalid", "CUSTRECORD_DJKK_ICL_INVENTORY_IN_CUSTODY", null), new nlobjSearchColumn("custrecord_djkk_icl_cusbin")// 預かり保管棚
            ]);
            ...
            if (!isEmpty(search_1)) {
                ...
                var search_3 = getSearchResults("customrecord_djkk_details_in_the_library", null, [["custrecord_djkk_ditl_quantity_possible", "greaterthan", "0"], "AND", search_3_select], [new nlobjSearchColumn("custrecord_djkk_ditl_newseriallot_number"), new nlobjSearchColumn("internalid"), new nlobjSearchColumn("internalid", "CUSTRECORD_DJKK_INVENTORY_DETAILS", null), new nlobjSearchColumn("custrecord_djkk_ditl_quantity_possible"),
