...
//	    		   new nlobjSearchColumn("city","shippingAddress",null), //市町村
	    ...
	    var transaction = defaultEmpty(record.getFieldValue('transactionnumber'));
	    //CH762 20230811 add by zdj end
	    //20230901 add by CH762 start 
        var CABINET_IMPORT_ADDRESS = '';
        if(subsidiary == SUB_NBKK){
            CABINET_IMPORT_ADDRESS = FILE_CABINET_ID_IMPORT_PDF_NBKK;
        }else if(subsidiary == SUB_ULKK){
            CABINET_IMPORT_ADDRESS = FILE_CABINET_ID_IMPORT_PDF_ULKK;
        }else{
            CABINET_IMPORT_ADDRESS = FILE_CABINET_ID_IMPORT_PDF;
        }
    ...
    var xmlString = '<?xml version="1.0"?><!DOCTYPE pdf PUBLIC "-//big.faceless.org//report" "report-1.1.dtd">\n' +
        ...
        '                        <img src="'+SECURE_URL_HEAD+'/core/media/media.nl?id=15969&amp;'+URL_PARAMETERS_C+'&amp;h=xwGkaOObH6n1hx7iEIKK7IzXqcP3XDaiz3GzyhnaY1td5xCX"\n' +
...
//    xlsFile.setName('仕入先返品(輸入)PDF印刷' + '_' + getFormatYmdHms() + '.pdf');
    xlsFile.setName('仕入先返品(輸出)PDF印刷' + '_' + transaction + '_' + getDateYymmddFileName() + '.pdf');
    //CH762 20230811 add by zdj start
    //20230901 add by CH762 start
//    xlsFile.setFolder(FILE_CABINET_ID_IMPORT_PDF);
    xlsFile.setFolder(CABINET_IMPORT_ADDRESS);
    //20230901 add by CH762 end
