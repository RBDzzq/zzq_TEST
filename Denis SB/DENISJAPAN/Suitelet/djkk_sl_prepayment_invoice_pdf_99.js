...
function suitelet(request, response) {
    ...
    try {
        ...
        var entity = salesorderRecord.getFieldValue('entity');// ŒÚ‹q
        var customerSearch = nlapiSearchRecord(
                ...
                [
                 ...
                 [
                  ...
                  new nlobjSearchColumn("currency"),  //Šî–{’Ê‰İ
                  ]
                );    
        ...
        var custLanguage = defaultEmpty(salesorderRecord.getFieldValue('custbody_djkk_language')); // ¿‹‘Œ¾Œê
        var trandate = defaultEmpty(salesorderRecord.getFieldValue('trandate')); // ¿‹‘“ú•t
        var otherrefnum = defaultEmpty(salesorderRecord.getFieldValue('otherrefnum')); // ”­’‘”Ô†
        var delivery_date = defaultEmpty(salesorderRecord.getFieldValue('custbody_djkk_delivery_date')); // ¿‹‘”[•i“ú
        var tranid = defaultEmpty(salesorderRecord.getFieldValue('tranid')); // ¿‹‘”Ô†
        var createdfrom = defaultEmpty(salesorderRecord.getFieldText('createdfrom')); // ¿‹‘ì¬Œ³
        var payment = defaultEmpty(salesorderRecord.getFieldText('custbody_djkk_payment_conditions')); // ¿‹‘x•¥ğŒ
        var salesrep = defaultEmpty(salesorderRecord.getFieldText('salesrep')); // ‰c‹Æ’S“–Ò
        var memo = defaultEmpty(salesorderRecord.getFieldValue('memo')); //memo
        var currency= defaultEmpty(isEmpty(customerSearch) ? '' :  customerSearch[0].getText("currency"));//”Ì”„ˆõi“–Ğ’S“–j
        var subsidiary = defaultEmpty(salesorderRecord.getFieldValue('subsidiary')); //¿‹‘q‰ïĞ
        ...
        var incoicedelivery_destination = salesorderRecord.getFieldValue('custbody_djkk_delivery_destination'); // ¿‹‘”[•iæ
        if (!isEmpty(incoicedelivery_destination)) {
            var invDestinationSearch = nlapiSearchRecord("customrecord_djkk_delivery_destination", null, [["internalid", "anyof", incoicedelivery_destination]], [new nlobjSearchColumn("custrecord_djkk_zip"), // —X•Ö”Ô†
    ...
    var invoiceCount = salesorderRecord.getLineItemCount('item');
        for (var k = 1; k < invoiceCount + 1; k++) {
            ...
            var invoiceItemSearch = nlapiSearchRecord("item", null, [["internalid", "anyof", invoiceItemId], ], [new nlobjSearchColumn("itemid"), // ¤•iƒR[ƒh
            new nlobjSearchColumn("displayname"), // ¤•i–¼
            new nlobjSearchColumn("custitem_djkk_product_name_jpline1"), // DJ_•i–¼i“ú–{ŒêjLINE1
            new nlobjSearchColumn("custitem_djkk_product_name_jpline2"), // DJ_•i–¼i“ú–{ŒêjLINE2
            new nlobjSearchColumn("custitem_djkk_product_name_line1"), // DJ_•i–¼i‰pŒêjLINE1
            new nlobjSearchColumn("custitem_djkk_product_name_line2"), // DJ_•i–¼i‰pŒêjLINE2
            ...
            var invoiceQuantity = defaultEmpty(salesorderRecord.getLineItemValue('item', 'quantity', k));// ”—Ê
            var invoiceRateFormat = defaultEmpty(salesorderRecord.getLineItemValue('item', 'rate', k));// ’P‰¿
            var invoiceAmount = defaultEmpty(parseFloat(salesorderRecord.getLineItemValue('item', 'amount', k)));// ‹àŠz
            ...
            var invoiceTaxrate1Format = defaultEmpty(salesorderRecord.getLineItemValue('item', 'taxrate1', k));// Å—¦
            var invoiceTaxamount = defaultEmpty(parseFloat(salesorderRecord.getLineItemValue('item', 'tax1amt', k)));// ÅŠz
            var invoiceTaxcode = defaultEmpty(salesorderRecord.getLineItemValue('item', 'taxcode', k));// Å—¦
            ...
            var invoiceUnitabbreviation = defaultEmpty(salesorderRecord.getLineItemValue('item', 'units_display', k));// ’PˆÊ
            itemLine.push({
...
function dealFugou(value) {
    var reValue = '';
    reValue = value.replace(new RegExp('&', 'g'), '&amp;').replace(new RegExp('&amp;lt;', 'g'), '&lt;') // [&]‚ğ’u‚«Š·‚¦‚é‚ÆAŒ³XƒGƒXƒP[ƒvˆ—‚³‚ê‚Ä‚¢‚é[<]‚ª•Ï‰»‚·‚é‚½‚ßA–ß‚·
