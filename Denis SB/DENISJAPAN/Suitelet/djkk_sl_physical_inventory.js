...
function createForm(request, response,div){
    ...
    var invCountYou = request.getParameter('invcntFlg');
    if (!invCountYou) {
        invCountYou = 'F';
    }
	...
	if(div == '1') {
	    ...
	    var invCountYouField = form.addField('custpage_inv_count_you', 'checkbox', '棚卸カウント用','');
	}
	// CH425　zheng 20230619 end
	...
	if(div == '1'){
        ...
        if(selectFlg == 'T') {  
            ...
            invCountYouField.setDisplayType('inline');
            // CH425　zheng 20230619 end
        }
        ...
        invCountYouField.setDefaultValue(invCountYou);
        // CH425　zheng 20230619 end
	}
   ...
   var parent = '';
	...
	var til3='';
	 ...
	 if(div == '1'){
         ...
         if (invCountYou == 'T') {
             // COUNT1
             subList.addField('custpage_inv_stock_count1', 'text', 'COUNT1');
             // COUNT2
             subList.addField('custpage_inv_stock_count2', 'text', 'COUNT2');
         }
         // その他
...
//	 subList.addField('count_real', 'float', '実地数量').setDisplayType('entry'); 
	 ...
	 if(selectFlg == 'T'){
		 ...
		 if(div == '1'){
			 ...
			 if (locationValue) {
			     if (locationArr.indexOf(locationValue) == -1) {
			         locationArr.push(locationValue);
			     }
			 }
             if (itemFieldValue) {
...
//                     }
                     ...
                     try {
                          if (!isEmpty(fileId)) {
                              if (Number(count) <= 0) {
                                  if(tmpSub !=SUB_SCETI && tmpSub !=SUB_DPKK){
                                      count_real = setCountNew(item_id, inv_no, location_id, file, fileFromDic, invCountYou);    
                                  } else {
                                      count_real = setCountA(item_id,inv_no_id,location_id,file,div,subValue, invCountYou);
                                  }
					     ...
					     if(tmpSub !=SUB_SCETI && tmpSub !=SUB_DPKK){
					         if (Number(count) > 0) {
		                         if (invCountYou == 'F') {
		                             ...
		                             strCsv+='"' + item_id+ '","'+item_name+'","'+item_english+'","'+item_product_code+'","'+item_vendorname+'","'+item_perunitquantity+'","'+location_id+'","'+location+'","'+binnumber_id+'","'+binnumber+'","'+inv_no_id+'","'+inv_no+'","'+invWarehouseNumber+'","'+invAkerSerialNumber+'","'+expirationdate+'","'+item_remark+'","'+invStockUnit+'","'+vo_or_cu_id+'","'+vo_or_cu+'","'+item_brand+'","'+count_real+'","",""';
		                             strCsv+='\r\n';   
		                         }
					         }
					     } else {
...
//					 strCsv+="\r\n"
            ...
            if (invCountYou == 'T') {
                csvTitleList.push(',');
                csvTitleList.push('COUNT1');
                csvTitleList.push(',');
                csvTitleList.push('COUNT2');
            }
            csvTitleList.push('\r\n');
...
//					    new nlobjSearchColumn("internalid")
	 ...
	 if(selectFlg == 'T'){
		 ...
		 var csvDownUrl = "window.location.href='" + csvDown(xmlString, div, subValue) + "'";
		 // CH294 zheng 20230316 end
...
function csvDown(xmlString, div, subsidiary){
	try{
	    // CH294 zheng 20230316 start
	    var prefix = '';
	    if (div == '1') {
...
//	    var csvFileName = prefix + '_' + getFormatYmdHms() + '.csv';
	    var csvFileName = '';
	    if(locationValueNew){
	        csvFileName = prefix + '_' + locationValueNew + '_' + getDateYymmddFileName()+'.csv';
	    }else{
	        csvFileName = prefix + '_' + getDateYymmddFileName()+'.csv';
	    }
	    //CH762 20230815 add by zdj start 
...
function setCountNew(item_id, inv_no, location_id, file, resultDic, invCountYou){
    ...
    for(var i = 0 ; i  < fileArr.length ; i++){
        if(!isEmpty(fileArr[i])){
            ...
            if(tmpItemId == item_id && tmpLocId == location_id && inv_no == fileInvNo){
                if (invCountYou == 'T') {
                    return isEmpty(fileLine[20]) ? '' : fileLine[20];
                } else {
                    return isEmpty(fileLine[22]) ? '' : fileLine[22];   
                }
            }
        }  
    }
...
function getApprovalDatas (paramSubsidiary, paramItemList, paramLotList, paramLocList) {
    ...
    var searchFilters = [];
    searchFilters.push(["custrecord_djkk_body_shedunloading_list.custrecord_djkk_shedunloading_status", "noneof", "2","3"]);
...
function setCountA(item_id,inv_no_id,location_id,file,div,subValue, invCountYou){
         ...
         if(!isEmpty(fileArr[i])){
             ...
             if(fileLine[0] == item_id && fileLine[10] == inv_no_id && fileLine[6] == location_id){
                 if(div == '1'){
                     ...
                     return isEmpty(fileLine[12]) ? '' : fileLine[12];
                 }
...
function getBinInfo (sub, locationList) {
    ...
    var searchColumns = [];
    searchColumns.push(new nlobjSearchColumn("internalid","binNumber","MAX"));
    ...
    searchColumns.push(new nlobjSearchColumn("internalid","inventoryNumber","GROUP"));
    ...
    if (searchResults && searchResults.length > 0) {
        for ( var i = 0; i < searchResults.length; i++) {
            ...
            resultDic[tmpId] = tmpId + ' ' + tmpBinNumber;
        }
    }
