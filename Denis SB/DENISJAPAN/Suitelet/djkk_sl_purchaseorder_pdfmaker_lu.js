...
function suitelet(request, response){
	...
	var purchaseorderId = nlapiGetContext().getSetting('SCRIPT', 'custscript1');//purchaseOrderId取得
	var roleSub = nlapiGetContext().getSetting('SCRIPT', 'custscript2');//purchaseOrderId取得
...
//	var roleSub = defaultEmpty(employee.getFieldValue('subsidiary'));
	nlapiLogExecution('error', 'roleSub', roleSub);
	nlapiLogExecution('error', 'purchaseorderId', purchaseorderId);
				...
				if(itemDetails[j].itemType != 'subtotalitem'&&!isEmpty(itemDetails[j].itemType)){
					...
					if (tmpDetailCnt % 8 == 0) {
					    str += '<td style="width: 25px;">'+ prefixInteger(no, 2) +'0</td>'+
		                    ...
		                    '</table>'+
		                    '</td>'+
		                    '<td align="right"  style="border-bottom: 1px solid #499AFF;border-left: 1px solid #499AFF;">'+ itemDetails[j].quantity + '&nbsp;' + itemDetails[j].units_display + '</td>'+
		                    '<td align="right" style="border-bottom: 1px solid #499AFF;border-left: 1px solid #499AFF;width: 85px;">'+ itemDetails[j].origrate +'&nbsp;</td>'+
        ...
        if (tmpLoopCnt > 0) {
            for(var t2 = 0; t2 < tmpLoopCnt;t2++) {
                ...
                if (tmpLoopCnt - 1 == t2) {
                    ...
                    '</tr>'
                    str += '<tr border="0px">'
						+ '<td border="0px"  style="width: 20px;height: 10px;" colspan="1" rowspan="2"></td>'
//						+ '<td border="1px"  style="height: 10px;border-top:2px solid #499AFF; border-left: 2px solid #499AFF;"></td>'
//						+ '<td border="1px"  style="height: 10px;border-top:2px solid #499AFF;" colspan="3"></td>'
//						+ '<td border="1px"  style="border-top:2px solid #499AFF; border-right: 2px solid #499AFF;height: 10px;"></td>'
						+ '</tr>';
                } else {
                    str += '<tr style="line-height:12px">'+
                    '<td style="width: 25px;">&nbsp;</td>'+
...
//				+ '<td border="1px"  style="border-top:2px solid #499AFF; border-right: 2px solid #499AFF;height: 10px;"></td>'
    ...
    str += '<tr >'
//		+ '<td style="width: 20px;"></td>'
		+ '<td style="border-left: 2px solid #499AFF;border-right: 2px solid #499AFF;border-top:2px solid #499AFF;height: 75px;" colspan="5">';
