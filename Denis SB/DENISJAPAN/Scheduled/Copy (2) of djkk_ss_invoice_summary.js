...
//			custNowMonthPaymentArr[customerpaymentSearch[i].getValue("internalid","customer","GROUP")] = customerpaymentSearch[i].getValue("amountpaid",null,"SUM");
	...
	var parentFolder = FILE_CABINET_ID_TOTAL_INV_OUTPUT_FILE;
...
//		nlapiSubmitFile(xlsFile);
		...
		if(invoiceSumPeriodtype != '25' &&!isEmpty(mailTempleteObj)){
			fileName = mailTempleteObj.fileName;
	...
	if(!isEmpty(customrecord_djkk_invoice_summarySearch)){nlapiLogExecution('debug','test2','test2')
		for(var i = 0 ; i < customrecord_djkk_invoice_summarySearch.length ; i++){
			...
			if(customrecord_djkk_invoice_summarySearch[i].getValue("custentity_djkk_totalinv_pdfsend_flg","CUSTRECORD_DJKK_INV_CUSTOMER",null) == 'T'){
				file = nlapiLoadFile(customrecord_djkk_invoice_summarySearch[i].getValue("custrecord_djkk_inv_pdf"));
			}
...
function stringXml(sub,custom,inv,id){
    ...
    var hdTotalPage = 0;
    var hdMaxPage = 21;
    if (inv.length <= hdMaxPage) {
        hdTotalPage = 1;
    } else {
        hdTotalPage = Math.ceil(inv.length/hdMaxPage);
    }
...
//      '<tr><td style="height: 60px;"><img style="margin-left: 30px; height: 60px;" src="'+custom.com_logo+'" />' + 
        '<tr style="height: 2px;">'
        if(custom.subsidiary == SUB_NBKK){
            str += '<td style=";margin-right:30px;"><img src="'+SECURE_URL_HEAD+'/core/media/media.nl?id=480236&amp;'+URL_PARAMETERS_C+'&amp;h=I4MoOlKGNT5wClfzyGHDmnp273JBVQ9BPk9YazigRcR_dkkh" style="width:100px;height: 100px; position:absolute;top:-8px;left:184px;" /></td>'
        }else if(custom.subsidiary == SUB_ULKK){
            str+='<td style=";margin-right:30px;"><img src="'+SECURE_URL_HEAD+'/core/media/media.nl?id=480237&amp;'+URL_PARAMETERS_C+'&amp;h=fr6dWpla-OixbfSgxwZye-1GZrZbgtlIsaTCz9wIGzkSuBIO" style="width:100px;height: 100px; position:absolute;top:-2px;left:210px;" /></td>'
        }else{
...
//      '<td>支払方法：'+custom.payMentWay+'</td><td align="right"><totalpages/>頁</td>'+
		...
		'</macro>' +
		'<macro id="nlheader">' +
//		'<table cellspacing="0" cellpadding="5" style="width: 660px;">'+
		'<table  padding-top="-40px" cellspacing="0" cellpadding="5" style="width: 660px;">'+
//		'<tr padding-top="10px"><td colspan="4" class="bottom-line-2px" align="center"><span style="margin-left: 120px; font-size: 18px">' + custom.com_legalname + ' 請求明細書</span></td><td class="bottom-line-2px" style="width: 120px" align="right">' + formatDate(new Date()) + '&nbsp;&nbsp;#&nbsp;&nbsp; <pagenumber/></td></tr>'+
		'<tr padding-top="10px"><td colspan="4" class="bottom-line-2px" align="center"><span style="margin-left: 120px; font-size: 18px">' + custom.com_legalname + ' 請求明細書</span></td><td class="bottom-line-2px" style="width: 120px" align="right">' + formatDate(new Date()) + '&nbsp;明細#${page}</td></tr>'+
		'<tr>'+
		'<td>請求先:</td>'+
...
//20220707 changed by zhou end
        ...
        for (s1 = 1; s1 <= hdTotalPage; s1++) {
            str += '#page'+s1+' { header: newHeader !important; header-height: 0px !important;footer: nlfooter !important;footer-height: 100px !important; }';
        }
        var dCnt = 0;
        for (s2 = hdTotalPage + 1; s2 <= hdTotalPage + 30; s2++) {
            dCnt = dCnt + 1;
            str += '#page'+s2+' { header: nlheader !important; header-height: 60px !important; footer: emptyHeader !important; footer-height: 0px !important;}';
        }
		//20220801  changed by zdj end
        str += '</style>'+
		'</head>'+
		'<body padding="0.5in 0.4in 0.5in 0.6in" size="Letter">'+
		'<table padding-top="402px" border="0" cellpadding="1" cellspacing="1" style="width:660px;"><tr>'+
		//'<table border="0" cellpadding="1" cellspacing="1" style="width:660px;"><tr>'+
...
//			str +='<td align="right" >-${'+Math.abs(inv[i].inComeTaxIn)+'?string[",##0;; roundingMode=halfUp"]}</td>';
			...
			str +='<td align="right" >${'+inv[i].inComeTaxEx+'?string[",##0;; roundingMode=halfUp"]}</td>';
			str +='<td align="right" >${'+inv[i].inComeTax+'?string[",##0;; roundingMode=halfUp"]}</td>';
			str +='<td align="right" >${'+inv[i].inComeTaxIn+'?string[",##0;; roundingMode=halfUp"]}</td>';
		}
	...
	str += '<pbr/>'
	str += '<table padding-top="-40px" cellspacing="0" cellpadding="5" style="width: 660px;">'+
	'<tr padding-top="17px">'+
	'<td>月次請求書番号:</td>'+
	'<td colspan="2">'+id+'</td>'+
	'<td>&nbsp;</td><td>&nbsp;</td>'+
	...
	for(var x = 0; x <inv.length; x++){
		var invDetial = inv[x];
		nlapiLogExecution('debug', 'invDetial', JSON.stringify(invDetial));
		nlapiLogExecution('debug', 'invDetialt', JSON.stringify(invDetial.taxSubList));
//add by zhou 20220707 start
		var type = inv[x].type;
		nlapiLogExecution('debug', 'type', type);
//end
		//add by zhou 20230509_CH477 start
//		str += '<table style=" width: 660px; border-top: 3px solid #000000; ' + (inv.length == 1 || x == inv.length - 1 ? 'border-bottom: 3px solid black;' : '') +'" cellspacing="0" cellpadding="5">' +
		str += '<table style=" width: 660px; border-top: 3px solid #000000; ' + (inv.length == 1 || x == inv.length - 1 ? 'border-bottom: 3px solid black;' : '') +'" cellspacing="0" cellpadding="4">' +
		//add by zhou 20230509_CH477 end
		'<tr><td style="width: 20px">' + (x + 1) + '</td><td>' +
		'<table>'+
		'<tr>'+
		...
		'</tr>'+
		'<tr>'+
//		'<td style="width: 100px;">請求書番号: </td>'+
//		'<td style="width: 150px;">'+invDetial.code+'</td>' +
//		'<td style="width: 80px;">発行日:</td>'+
//		'<td>'+invDetial.issuedate+'</td>'+
//		'<td style="width: 80px;">備考：</td>' +
//		'<td>'+invDetial.memo+'</td>' +
//		'</tr>'+
//		'<tr>'+
//		'<td>受注番号: </td>'+
...
//		'<td>納品先: </td>'+
//		'<td colspan="3">'+invDetial.deliveryDestCode+ '&nbsp;' + invDetial.deliveryDest + '</td>'+
//		'</tr>'+
//		'<tr>'+
//		'<td>支払条件: </td>'+
//		'<td colspan="2">'+invDetial.termsofpayment+'</td>'+
//		'<td>'+(invDetial.postalcode ? '〒' + invDetial.postalcode : '')+'</td>'+
//		'<td>'+invDetial.prefectures+'</td>'+
//		'<td>'+invDetial.thecitydoesnot +invDetial.addressone+invDetial.addresstwo+invDetial.addressthree+'</td>'+
//		'</tr>'+
		//add by zhou 20230509_CH477 start
//		'<td style="width: 100px;">請求書番号: </td>'+
		'<td >請求書番号: </td>'+
		//add by zzq CH670 20230710 start 
		'<td >'+invDetial.code+'</td>'+
		'<td >営業担当者:</td>'+
		'<td colspan="3" align="left">'+invDetial.salesrepName+' '+invDetial.section+'</td>'+
//		'<td>'+invDetial.salesrepName+'</td>'+
//		'<td style="width: 80px;">セクション: </td>'+
//		'<td>'+invDetial.section+'</td>'+
		// add by zzq CH670 20230710 end
	'</tr>'+
	'<tr>'+
		'<td>受注番号: </td>'+
		'<td>'+invDetial.createfrom+'</td>'+
    ...
    '</tr>'+
    '<tr>'+
    //add by 20230511 zhou CH485 start
//		'<td >支払条件: </td>'+
//		'<td>'+invDetial.termsofpayment+'</td>'+
  //add by zzq CH670 20230710 start
//		'<td colspan="2"></td>'+
//		'<td>納品先: </td>'+
//		'<td colspan="3">'+invDetial.deliveryDestCode+ '&nbsp;' + invDetial.deliveryDest + '</td>'+
...
//		'<td colspan="4">'+(invDetial.postalcode ? '〒' + invDetial.postalcode : '')+ '&nbsp;'+invDetial.prefectures+ '&nbsp;'+ invDetial.thecitydoesnot+ '&nbsp;'+ invDetial.addressone+ '&nbsp;'+ invDetial.addresstwo+ '&nbsp;'+ invDetial.addressthree+'</td>'+
	//add by 20230511 zhou CH485 end
		//add by zzq CH670 20230710 end
	'</tr>'+
	//add by zzq CH670 20230710 start
//	'<tr>'+
//		'<td colspan="3"></td>'+
//		'<td>'+(invDetial.postalcode ? '〒' + invDetial.postalcode : '')+'</td>'+
//		'<td colspan="2">'+invDetial.prefectures+ '&nbsp;'+ invDetial.thecitydoesnot+ '&nbsp;'+ invDetial.addressone+ '&nbsp;'+ invDetial.addresstwo+ '&nbsp;'+ invDetial.addressthree+'</td>'+
////		'<td>'+invDetial.thecitydoesnot +invDetial.addressone+invDetial.addresstwo+invDetial.addressthree+'</td>'+
//	'</tr>'+
	//add by zzq CH670 20230710 end
  //add by zhou 20230509_CH477 end
		'<tr class="lalala"><td colspan="6"></td></tr>' +
		'</table>'+
//		'<table style="width: 640px;" cellspacing="0" cellpadding="5">'+
		'<table style="width: 640px;" cellspacing="0" cellpadding="2">'+
		'<tr>'+
		'<td style="width: 100px;" class="border-tb-dash">明&nbsp;&nbsp;細</td>'+
		//add by zhou 20230508_CH481 start
//		'<td style="width: 20px;" class="border-tb-dash">&nbsp;</td>'+
		'<td style="width: 300px;" align="right" class="border-tb-dash"></td>'+
//		'<td style="width: 100px;" align="right" class="border-tb-dash">数量</td>'+
		'<td style="width: 80px;" align="right" class="border-tb-dash">数量</td>'+
//		'<td style="width: 60px;" align="right" class="border-tb-dash">単価</td>'+
		'<td style="width: 80px;" align="right" class="border-tb-dash">単価</td>'+
//		'<td style="width: 60px;" align="right" class="border-tb-dash">金額</td>'+
		'<td style="width: 80px;" align="right" class="border-tb-dash">金額</td>'+
...
//		'<td align="right" class="border-tb-dash">金額</td>'+
		...
		for(var k=0; k<detailList.length; k++){
			...
			str +='<tr>';
			//add by zzq CH670 20230710 start 
//			str +='<td>'+dealFugou(detail.itemName)+'</td>';
//			str +='<td colspan="3">'+itemplcode+' '+dealFugou(detail.itemName)+'</td>';
			//str +='<td colspan="2">'+itemplcode+' '+dealFugou(detail.itemName)+'</td>';
			str +='<td>'+itemplcode+'</td>'+
			'<td>'+dealFugou(detail.itemName)+'</td>';
			//add by zzq CH670 20230710 end
			//add by zhou 20230524_CH575 start
			var taxCodeText = detail.taxCode == 11 ? '※' : '';
//			str +='<td> ※ </td>';
//			str +='<td>' +taxCodeText+ '</td>';
			//add by zhou 20230524_CH575 end
			//add by zhou 20230508_CH481 start
			//add by zhou 20230524_CH575 start
//			var taxCodeText = detail.taxCode == 11 ? '※軽減税率対象' : '';
//			str +='<td>'+taxCodeText+'</td>';
//			str +='<td></td>';
			//add by zhou 20230524_CH575 end
			//add by zhou 20230508_CH481 end
			//add by zzq CH670 20230710 start
//			str +='<td align="right">' +detail.itemCount + '&nbsp;' + detail.itemUnit + '</td>';
			var itemCount = detail.itemCount;
			if(itemCount){
			    itemCount = formatAmount1(detail.itemCount);
			}
//			str +='<td align="right">'+taxCodeText+' ' +detail.itemCount + '&nbsp;' + detail.itemUnit + '</td>';
			str +='<td align="right">'+taxCodeText+' ' +itemCount + '&nbsp;' + detail.itemUnit + '</td>';
			//add by zzq CH670 20230710 end
			if(detail.itemAmount<0){
				//add by zhou 20230522_CH580 start
				if(Math.abs(detail.itemPrice)==0){
			...
			//add by zzq CH670 20230710 start
//			str +='<tr>';
//			//add by zhou 20230607 CH631 start
//			if(detail.itemProductCode && !detail.itemLocationBarcode){
//			    str +='<td style="margin-top:-8px;" colspan="6">'+detail.itemProductCode+'</td>';
//	            str +='</tr>';
//			}else if(!detail.itemProductCode && detail.itemLocationBarcode){
//			    str +='<td style="margin-top:-8px;" colspan="6">'+detail.itemLocationBarcode+'</td>';
//                str +='</tr>';
//			}else if(detail.itemProductCode && detail.itemProductCode){
//			    str +='<td style="margin-top:-8px;" colspan="6">'+detail.itemProductCode+ '　'+detail.itemLocationBarcode+'</td>';
//                str +='</tr>';
//			}else{
//			    str +='<td style="margin-top:-8px;" colspan="6"></td>';
//                str +='</tr>';
//			}
			//add by zzq CH670 20230710 end
//			str +='<tr>';
//			str +='<td colspan="6">'+detail.itemLocationBarcode+'</td>';
//			str +='</tr>';
			//add by zhou 20230607 CH631 end
		}
		str +='</table>'+
...
//		'</tr>';
		...
		if(type != 'クレジットメモ'){
			...
			for(var taxType in invDetial.taxSubList){
				...
				str += '<tr>'+
					'<td></td>';
				    nlapiLogExecution('debug','taxType',taxType)
				    //add by zhou 20230508_CH478 start
//					str +='<td>-' +taxType+ '</td>';
				    str +='<td>' +taxType+ '</td>';
				  //add by zhou 20230508_CH478 end
...
//				'<td>&nbsp;</td>'+
				'<td>※軽減税率対象</td>'+
				'<td class="border-top-dot"> 合計 </td>'+
				'<td class="border-top-dot" align="right">${'+Math.abs(invDetial.totalSum)+'?string[",##0;; roundingMode=halfUp"]}</td>'+
...
//				'<table style="width: 640px;" cellspacing="0" cellpadding="5">'+
//				'<tr>'+
//                '<td style="width: 300px;"> ※軽減税率対象 </td>'+
//                '</tr></table>'+
              //add by zhou 20230524_CH575 end
				//add by zzq CH670 20230710 end
				'</td></tr></table>';
...
//				'</td></tr></table>';
	...
	//add by zhou 20230524_CH575 start
//	str += '<table style=" width: 660px; border-top: 3px solid #000000; " cellspacing="0" cellpadding="4">' +
//	'<tr><td style="width: 20px"> ※軽減税率対象 </td></tr>' +
//	'</table>';
	//add by zhou 20230524_CH575 end
		str += '</body>'+
		'</pdf>';
    ...
    for(var i =0;i<totalinvNumArr.length;i++){
        ...
        if(!isEmpty(summarySearch)){
            ...
            for(var cd = 0 ; cd < summarySearch.length ; cd++){
                //DJ_pdf表示
                var pdfFileId = summarySearch[cd].getValue("custrecord_djkk_inv_pdf");
