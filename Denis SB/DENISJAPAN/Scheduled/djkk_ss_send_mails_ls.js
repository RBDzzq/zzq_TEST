...
function scheduled(type) {
	...
	var sendmailtype = nlapiGetContext().getSetting('SCRIPT',
			'custscript_djkk_sendmail_type_ls');

	// 会社
	var mainsubsidiary = nlapiGetContext().getSetting('SCRIPT',
			'custscript_djkk_send_mail_subsidiary_ls');
	...
	if (sendmailtype == '出荷予実比較レポート送信') {
		...
		var infoSearchone = nlapiSearchRecord(
				"customrecord_djkk_mail_send_info",									//DJ_メール送信情報
				null,
				[
						...
						[ "formulatext: {custrecord_djkk_mail_kinou_name}",
								"contains", "出荷予実比較レポート送信機能_1Week" ] ],	//メール_機能名--is出荷予実比較レポート送信機能_1Week
				[
		...
		var fileoneweek = outReportOneWeek(mainsubsidiary);							//一週間の場合

		var infoObjOne = {
			'sender' : infoSearchone[0].getValue('custrecord_djkk_mail_sender'),	//差出人ID
			'recipients' : infoSearchone[0]
					.getValue('custrecord_djkk_mail_recipients'),					//宛先内部ID
			'title' : infoSearchone[0].getValue('custrecord_djkk_mail_title')		//タイトル
		};

		// 送信
		sndActualForecastMail(true, fileoneweek, infoObjOne);						//送信
		...
		var infoSearchfour = nlapiSearchRecord(
				...
				null,
				[
						[ "custrecord_djkk_mail_subsidiary", "anyof",				//連結分類
								mainsubsidiary ],
						"AND",
						[ "formulatext: {custrecord_djkk_mail_kinou_name}",			//メール_機能名--is出荷予実比較レポート送信機能_1Week
		...
		var filefourweek = outReportFourWeek(mainsubsidiary);						//一週間の場合

		var infoObjFour = {
			'sender' : infoSearchfour[0]
					.getValue('custrecord_djkk_mail_sender'),						//差出人ID
			'recipients' : infoSearchfour[0]
					.getValue('custrecord_djkk_mail_recipients'),					//宛先内部ID
			'title' : infoSearchfour[0].getValue('custrecord_djkk_mail_title')		//タイトル
		};

		// 送信
		sndActualForecastMail(false, filefourweek, infoObjFour);					//送信
		...
		dataObj['headertime'] = nlapiDateToString(getSystemTime());					//現在のシステム時間

		var fcmainSearch = nlapiSearchRecord("customrecord_djkk_sc_forecast_ls",	//DJ_SC課FC_LS
				null, [
						[ "isinactive", "is", "F" ],								//active??
						"AND",
						[ "lastmodified", "within", lastmodified ],					//日付
						"AND",
						[ "custrecord_djkk_sc_fc_ls_subsidiary", "anyof",			//DJ_連結
								mainsubsidiary ] ], [
						new nlobjSearchColumn("lastmodifiedby", null, "GROUP"),
						new nlobjSearchColumn("class",
								"custrecord_djkk_sc_fc_ls_item", "GROUP"),			//DJ_商品コード
						new nlobjSearchColumn(
								"custrecord_djkk_sc_fc_ls_location_area", null,		//DJ_場所エリア
								"GROUP") ]);
		if (!isEmpty(fcmainSearch)) {

			var headerAry = [];

			for (var i = 0; i < fcmainSearch.length; i++) {							//重要な情報を保存する		

				var lastmodifiedby = fcmainSearch[i].getText("lastmodifiedby",		
						null, "GROUP");
				var classValue = fcmainSearch[i].getText("class",					
						"custrecord_djkk_sc_fc_ls_item", "GROUP");
				var locationArea = fcmainSearch[i].getText(
						"custrecord_djkk_sc_fc_ls_location_area", null, "GROUP");	
//				nlapiLogExecution('debug', '日付pe', lastmodifiedby);
//				nlapiLogExecution('debug', 'DJ_商品コード', classValue);
//				nlapiLogExecution('debug', 'DJ_場所エリア', locationArea);
				headerAry.push({
		...
		var forecastSearchdetil = nlapiSearchRecord(
				"customrecord_djkk_sc_forecast_ls",									//DJ_SC課FC_LS
				null,
				[
						[ "isinactive", "is", "F" ],
						"AND",
						...
						"AND",
						[ "custrecord_djkk_sc_fc_ls_subsidiary", "anyof",
								mainsubsidiary ] ],
				[
						new nlobjSearchColumn("itemid",
								"custrecord_djkk_sc_fc_ls_item", "GROUP"),			//商品情報
						new nlobjSearchColumn("displayname",
								"custrecord_djkk_sc_fc_ls_item", "GROUP"),			//
						new nlobjSearchColumn(
								"custrecord_djkk_sc_fc_ls_location_area", null,		//DJ_場所エリア
								"GROUP"),
						new nlobjSearchColumn("custrecord_djkk_sc_fc_ls_in", null,	//	DJ_IN
								"SUM"),
						new nlobjSearchColumn(										//DJ_開始日
								"custrecord_djkk_sc_fc_ls_startdate", null, "MIN"),
						new nlobjSearchColumn("custrecord_djkk_sc_fc_ls_enddate",	//DJ_終了日
								null, "MAX"),
						new nlobjSearchColumn("formulanumeric", null, "SUM")		//
								.setFormula("1"),
						new nlobjSearchColumn(										//	DJ_連結
								"custrecord_djkk_sc_fc_ls_subsidiary", null,
								"GROUP") ]);
		if (!isEmpty(forecastSearchdetil)) {
			...
			for (var i = 0; i < forecastSearchdetil.length; i++) {
				var column = forecastSearchdetil[0].getAllColumns();				//商品な情報を保存する		
		...
		var forecastsalemanSearch = nlapiSearchRecord(
				"customrecord_djkk_sc_forecast_ls",
				null,
				[
						[ "isinactive", "is", "F" ],
						"AND",
				...
				[
						...
						new nlobjSearchColumn("displayname",
								"custrecord_djkk_sc_fc_ls_item", "GROUP"),
						new nlobjSearchColumn(
								"custrecord_djkk_sc_fc_ls_location_area", null,
								"GROUP"),
						new nlobjSearchColumn("custrecord_djkk_sc_fc_ls_in", null,
								"SUM"),
						new nlobjSearchColumn(
								"custrecord_djkk_sc_fc_ls_startdate", null, "MIN"),
						new nlobjSearchColumn("custrecord_djkk_sc_fc_ls_enddate",
								null, "MAX"),
						new nlobjSearchColumn("formulanumeric", null, "SUM")
								.setFormula("1"),
						new nlobjSearchColumn("lastmodifiedby", null, "GROUP"),
		...
		var infoSearchdaliy = nlapiSearchRecord(
				...
				null,
				[
						[ "custrecord_djkk_mail_subsidiary", "anyof",				//連結
								mainsubsidiary ],
						"AND",
						[ "formulatext: {custrecord_djkk_mail_kinou_name}",			//メール_機能名
		...
		var infoObj = {
			'sender' : infoSearchdaliy[0]
					.getValue('custrecord_djkk_mail_sender'),						//差出人
			'recipients' : infoSearchdaliy[0]
					.getValue('custrecord_djkk_mail_recipients'),					//宛先
			'title' : infoSearchdaliy[0].getValue('custrecord_djkk_mail_title')		//タイトル
		};

		sndDailyNewsMail(dataObj, infoObj);											//送信
		...
		var forecastSearch = nlapiSearchRecord("customrecord_djkk_sc_forecast_ls",null,
				[
				   ["custrecord_djkk_sc_fc_ls_startdate","onorafter","today"], 
				   "AND", 
				   ["formulanumeric: TO_NUMBER({custrecord_djkk_sc_fc_ls_balance})","notgreaterthanorequalto","0"], 
				   "AND", 
				   ["custrecord_djkk_sc_fc_ls_subsidiary","anyof",mainsubsidiary]
				], 
				[
				   new nlobjSearchColumn("class","custrecord_djkk_sc_fc_ls_item","GROUP"), 
				   new nlobjSearchColumn("itemid","custrecord_djkk_sc_fc_ls_item","GROUP"), 
				   new nlobjSearchColumn("displayname","custrecord_djkk_sc_fc_ls_item","GROUP"), 
				   new nlobjSearchColumn("formulatext",null,"GROUP").setFormula("'*'"), 
				   new nlobjSearchColumn("custrecord_djkk_sc_fc_ls_yearmonthweek",null,"GROUP").setSort(false), 
				   new nlobjSearchColumn("custrecord_djkk_sc_fc_ls_subsidiary",null,"GROUP"), 
				   new nlobjSearchColumn("custrecord_djkk_sc_fc_ls_startdate",null,"GROUP"), 
	...
	//U067 営業計画情報入力送信 
			...
			var fcmainSearch = nlapiSearchRecord("customrecord_djkk_so_forecast_ls",
					null, [
							[ "isinactive", "is", "F" ],
							"AND",
							[ "lastmodified", "within", lastmodified ],
							"AND",
							[ "custrecord_djkk_so_fc_ls_subsidiary", "anyof",
									mainsubsidiary ] ], [
							new nlobjSearchColumn("lastmodifiedby", null, "GROUP"),
							new nlobjSearchColumn("class",
									"CUSTRECORD_DJKK_SO_FC_LS_ITEM", "GROUP"),
							new nlobjSearchColumn(
									"custrecord_djkk_so_fc_ls_location_area", null,
									"GROUP")
							]);
			...
			var lineAry2 = [];
			var forecastSearchdetil = nlapiSearchRecord(
					"customrecord_djkk_so_forecast_ls",
					null,
					[
							[ "isinactive", "is", "F" ],
							"AND",
					...
					[
							...
							new nlobjSearchColumn("displayname","CUSTRECORD_DJKK_SO_FC_LS_ITEM", "GROUP"),
...
//				var lineAry = [];
				...
				for (var i = 0; i < forecastSearchdetil.length; i++) {
					...
					lineAry.push({
						...
						'subsidiary' : forecastSearchdetil[i].getText(column[2]),
						'locationArea' : forecastSearchdetil[i].getText(column[3]),
//						'employee' : forecastSearchdetil[i].getText(column[4]),
						'year' : forecastSearchdetil[i].getValue(column[5]),
						'month' : forecastSearchdetil[i].getValue(column[6]),
						'week' : forecastSearchdetil[i].getValue(column[7]),
						'yearmonthweek' : forecastSearchdetil[i].getValue(column[8]),
						'fcnum' : forecastSearchdetil[i].getValue(column[9]),
						'weekFcnum' : forecastSearchdetil[i].getValue(column[10])
					});
				}
			...
			var forecastSearchdetil = nlapiSearchRecord(
					...
					[
							...
							new nlobjSearchColumn('custrecord_djkk_so_fc_ls_week', null,"GROUP"),
							new nlobjSearchColumn('custrecord_djkk_so_fc_ls_yearmonthweek', null,"GROUP"),
							new nlobjSearchColumn('custrecord_djkk_so_fc_ls_fcnum', null,"GROUP"),
							new nlobjSearchColumn('custrecord_djkk_so_fc_ls_week_fcnum', null,"GROUP")
			]);
			if (!isEmpty(forecastSearchdetil)) {
//				var lineAry2 = [];
...
function outReportOneWeek(mainsubsidiary) {
	...
	var dataObj = getFCLineDate(searchstartDate, searchendDate, mainsubsidiary,
			[[ "custrecord_djkk_sc_fc_ls_enddate", "on", searchendDate ]]);
...
function outReportFourWeek(mainsubsidiary) {
	...
	var dataObj = getFCLineDate(searchstartDate, searchendDate, mainsubsidiary,	[
	                                                                      	   ["custrecord_djkk_sc_fc_ls_enddate","on",searchendDate], 
	                                                                    	   "OR", 
	                                                                    	   ["custrecord_djkk_sc_fc_ls_enddate","on",dateAddDays(searchendDate, -7)], 
	                                                                    	   "OR", 
	                                                                    	   ["custrecord_djkk_sc_fc_ls_enddate","on",dateAddDays(searchendDate, -14)], 
	                                                                    	   "OR", 
	                                                                    	   ["custrecord_djkk_sc_fc_ls_enddate","on",dateAddDays(searchendDate, -21)]
	                                                                    	]);
	...
	var sc_forecastSearch = nlapiSearchRecord("customrecord_djkk_sc_forecast_ls",
			null, [
					[ "custrecord_djkk_sc_fc_ls_enddate", "on", searchendDate ],
					"OR",
					[ "custrecord_djkk_sc_fc_ls_enddate", "on",
					...
					new nlobjSearchColumn("custrecord_djkk_sc_fc_ls_item", null,
							"GROUP"),
					new nlobjSearchColumn("custrecord_djkk_sc_fc_ls_in", null,
							"SUM"),
					new nlobjSearchColumn("custrecord_djkk_sc_fc_ls_subsidiary",
							null, "GROUP") ]);
...
function getFCLineDate(searchstartDate, searchendDate, mainsubsidiary,
	...
	var forecastSearch = getSearchResults("customrecord_djkk_sc_forecast_ls",
			null, searchendDatesfilter, [
					new nlobjSearchColumn("class","custrecord_djkk_sc_fc_ls_item", "GROUP"),
					new nlobjSearchColumn("itemid","custrecord_djkk_sc_fc_ls_item", "GROUP"),
					new nlobjSearchColumn("displayname","custrecord_djkk_sc_fc_ls_item", "GROUP"),
					new nlobjSearchColumn("formulanumeric",null,"SUM").setFormula("{custrecord_djkk_sc_fc_ls_in}-{custrecord_djkk_sc_fc_ls_out}"), 
					new nlobjSearchColumn("custrecord_djkk_sc_fc_ls_subsidiary",null,"GROUP"),
					new nlobjSearchColumn("formulatext",null,"GROUP").setFormula("{custrecord_djkk_sc_fc_ls_item.internalid}||'-'||{custrecord_djkk_sc_fc_ls_subsidiary.internalid}") ]);
	if (!isEmpty(forecastSearch)) {
		...
		for (var i = 0; i < forecastSearch.length; i++) {
			...
			if (forecastSearch[i].getValue(column[4]) == mainsubsidiary) {
				nlapiLogExecution('debug', 'mainsubsidiary', mainsubsidiary);
...
//				}
					nlapiLogExecution('debug','lineDataone',lineData.length);
...
function sndActualForecastMail(weekFlg, data, infoSearchone) {
	...
	if (weekFlg) {
		...
		if (!isEmpty(data)) {
			...
			//nlapiLogExecution('debug','lineData',lineData.length);
...
function sndBPInformationMail(dataObj, infoObj) {
	nlapiLogExecution('debug','sendMali start','start')
	if (!isEmpty(dataObj)) {
		...
		dailyNewsStr += '        <th width="55px">DJ_週数</th>';
		dailyNewsStr += '        <th width="55px">DJ_年月週</th>';
		dailyNewsStr += '        <th width="85px">DJ_月FC数</th>';
		dailyNewsStr += '        <th width="85px">DJ_週FC数</th>';
	    ...
	    if(!isEmpty(lineAry)){
	    	for (var a = 0; a < lineAry.length; a++) {
	    		var laObj = lineAry[a];
				dailyNewsStr += '    <tr>';
				dailyNewsStr += '        <td>' + laObj['subsidiary'] + '</td>';
				...
				dailyNewsStr += '        <td>' + laObj['week'] + '</td>';
				dailyNewsStr += '        <td>' + laObj['yearmonthweek'] + '</td>';
				dailyNewsStr += '        <td>' + laObj['fcnum'] + '</td>';
				dailyNewsStr += '        <td>' + laObj['weekFcnum'] + '</td>';
				dailyNewsStr += '    </tr>';
	    	}
	    }
		dailyNewsStr += '</table>';
		...
		if(!isEmpty(lineAry2)){
			for (var b = 0; b < lineAry2.length; b++) {
				...
				dailyNewsStr += '        <td>' + la2Obj['subsidiary'] + '</td>';
				...
				dailyNewsStr += '        <td>' + la2Obj['week'] + '</td>';
				dailyNewsStr += '        <td>' + la2Obj['yearmonthweek'] + '</td>';
				dailyNewsStr += '        <td>' + la2Obj['fcnum'] + '</td>';
				dailyNewsStr += '        <td>' + la2Obj['weekFcnum'] + '</td>';
				dailyNewsStr += '    </tr>';
			}
		}
		dailyNewsStr += '</table>';
