 ...
 */
define(['N/record', 'N/ui/serverWidget', 'N/runtime', 'N/search', 'N/redirect', 'SuiteScripts/DENISJAPAN/2.0/Common/djkk_common'], function(record, serverWidget, runtime, search, redirect, djkk_common) {
    ...
    function beforeLoad(scriptContext) {
        ...
        if (scriptContext.type == 'edit') {
            ...
            if (apprDealFlg) {
                ...
                // 承認後編集できる
                var apprModifyFlg = ((subsidiary == 2 || subsidiary == 4) && apprStatus == '2' && (recType == 'purchaseorder'));
                if (!(applyFlg || appr1Flg || appr2Flg || appr3Flg || appr4Flg || apprModifyFlg)) {
                    redirect.toRecord({
                        type : recType,
                        id : recId,
                        isEditMode : false
    ...
    function beforeSubmit(scriptContext) {
        ...
        if (runtime.executionContext == runtime.ContextType.USER_INTERFACE) {
            if (type == 'create') {
                ...
                if (curRec.type == 'purchaseorder') {
                    ...
                    var apprFlag = curRec.getValue('custbody_djkk_trans_appr_deal_flg');
                    if (!apprFlag) {
                        // CH809 zheng 20230815 start
                        // curRec.setValue({
                        // fieldId : 'approvalstatus',
                        // value : '2'
                        // });
                        // CH809 zheng 20230815 end
                    }
                    // 顧客返品
        ...
        // add by CH793 20230830 zzq start
        if (type == 'create') {
            if (curRec.type == 'invoice') {
                var recType = curRec.type;
                // 作成元
                var createdfrom = curRec.getValue('createdfrom');
                log.error('zzq_createdfrom', createdfrom);
                // 子会社
                var subsidiary = curRec.getValue('subsidiary');
                log.error('zzq_subsidiary', subsidiary);
                if (createdfrom) {
                    var createdfromObjs = djkk_common.lookupFields('transaction', createdfrom, ['type']);
                    log.error('zzq_createdfromObjs', createdfromObjs);
                    var createdfromType = createdfromObjs.type_value;
                    if (createdfromType == 'SalesOrd') {
                        var salesorderObjs = djkk_common.lookupFields('salesorder', createdfrom, ['custbody_djkk_trans_appr_create_user']);
                        // log.error('zzq_salesorderObjs', salesorderObjs);
                        // DJ_外部システム連携_注文番号
                        var exsystem_tranid = '';
                        // DJ_作成者
                        var create_user = '';
                        if (salesorderObjs) {
                            // exsystem_tranid = salesorderObjs.custbody_djkk_exsystem_tranid_value;
                            exsystem_tranid = curRec.getValue('custbody_djkk_exsystem_tranid');
                            log.error('zzq_exsystem_tranid', exsystem_tranid);
                            create_user = salesorderObjs.custbody_djkk_trans_appr_create_user_value;
                            // create_user = curRec.getValue('custbody_djkk_trans_appr_create_user');
                            log.error('zzq_create_user', create_user);
                        }
                        if (exsystem_tranid) {
                            // 承認対象
                            var approvalObj = getApprovalObj(recType);
                            log.error('zzq_approvalObj', approvalObj);
                            var fieldList = FIELD_MAPPING[approvalObj];
                            // 承認情報をクリアする
                            fieldClear(fieldList, curRec);
                            var currentRole = '';
                            if (subsidiary == '2') {
                                currentRole = '1013';
                            ...
                            if (currentRole) {
                                ...
                                var dealKbn = '1';
                                var resultObj = getDealRole(subsidiary, approvalObj, dealKbn, currentRole);
                                log.error('zzq_resultObj', JSON.stringify(resultObj));
                                if (Object.keys(resultObj).length > 0) {
                                    if (resultObj.createRole == currentRole) {
                                        // DJ_承認処理フラグ
                                        curRec.setValue({
