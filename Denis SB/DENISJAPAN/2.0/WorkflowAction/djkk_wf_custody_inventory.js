...
define(['N/runtime', 'N/search', 'N/redirect'], function(runtime, search, redirect) {
    ...
    var FIELD_MAPPING = {};
    var DETAIL_LIST = [];
    DETAIL_LIST.push('custrecord_djkk_body_flg');//DJ_承認処理フラグ
    DETAIL_LIST.push('custrecord_djkk_body_createrole');//DJ_作成ロール
    DETAIL_LIST.push('custrecord_djkk_body_createuser');//DJ_作成者
    DETAIL_LIST.push('custrecord_djkk_body_next_role');//DJ_次の承認ロール
    DETAIL_LIST.push('custrecord_djkk_body_next_condi');//DJ_次の承認条件

    //預かり在庫実地棚卸承認画面
    FIELD_MAPPING['14'] = DETAIL_LIST
    /**
...
//        var oldRecord = scriptContext.oldRecord
        ...
        var subsidiary =  newRecord.getValue('custrecord_djkk_body_sub'); //DJ_連結
        var createrole =  newRecord.getValue('custrecord_djkk_body_createrole'); 
        ...
        var approvalObj = script.getParameter({
            name : 'custscript_djkk_custodyl_wf_obj'
        });

        // 処理区分
        var dealKbn = script.getParameter({
            name : 'custscript_djkk_custody_kbn'
        });
