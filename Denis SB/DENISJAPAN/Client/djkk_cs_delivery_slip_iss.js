...
function clientFieldChanged(type, name, linenum) {
	if(name == 'custpage_subsidiary'){
		...
		var https = nlapiResolveURL('SUITELET', 'customscript_djkk_sl_delivery_slip_iss', 'customdeploy_djkk_sl_delivery_slip_iss');
...
function search(){
	// changed by song add CH326 start
	var locationValue = nlapiGetFieldValue('custpage_location');
	if(isEmpty(locationValue)){
		alert('「DJ_場所」選択してください。')
		return false;
	//changed by song add CH326 end
	}else{
		var parameter = setParam();
...
function setParam(){
	...
	parameter += '&soNo='+nlapiGetFieldValue('custpage_so_no');	
	parameter += '&subsidiary='+nlapiGetFieldValue('custpage_subsidiary');
	parameter += '&location='+nlapiGetFieldValue('custpage_location');
	// changed by song add CH326 start	
	parameter += '&ordertype='+nlapiGetFieldValue('custpage_ship_order');
	parameter += '&issueflg='+nlapiGetFieldValue('custpage_ship_issue');
	// changed by song add CH326 end	
	return parameter;
}
