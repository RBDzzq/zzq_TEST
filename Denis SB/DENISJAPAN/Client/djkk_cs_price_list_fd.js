...
function clientLineInit(type){
	var sub=nlapiGetFieldValue('custrecord_djkk_price_subsidiary_fd');
	var dtSub = nlapiGetCurrentLineItemValue('recmachcustrecord_djkk_pldt_pl_fd', 'custrecord_djkk_price_subsidiary_pl_fd');
	// CH648 zheng 20230607 start
	if (dealType == 'edit' || dealType == 'copy') {
	    
	    // DJ_ブランド
        var dtBrand = nlapiGetCurrentLineItemValue('recmachcustrecord_djkk_pldt_pl_fd', 'custrecord_djkk_pldt_brand');
        // DJ_商品コード
        var dtItemCode = nlapiGetCurrentLineItemValue('recmachcustrecord_djkk_pldt_pl_fd', 'custrecord_djkk_pldt_itemcode_fd');
        // DJ_仕入先
        var dtSupplier = nlapiGetCurrentLineItemValue('recmachcustrecord_djkk_pldt_pl_fd', 'custrecord_djkk_pldt_supplier_fd');
        // DJ_商品名(日本語)
        var dtItemNameJp = nlapiGetCurrentLineItemValue('recmachcustrecord_djkk_pldt_pl_fd', 'custrecord_djkk_pldt_itemname_jp_fd');
        // DJ_商品名(英語)
        var dtItemnameEn = nlapiGetCurrentLineItemValue('recmachcustrecord_djkk_pldt_pl_fd', 'custrecord_djkk_pldt_itemname_en_fd');
        // DJ_カタログコード
        var dtCatalogCode = nlapiGetCurrentLineItemValue('recmachcustrecord_djkk_pldt_pl_fd', 'custrecord_djkk_pldt_catalog_code');
        // DJ_JANコード
        var dtJanCode = nlapiGetCurrentLineItemValue('recmachcustrecord_djkk_pldt_pl_fd', 'custrecord_djkk_pldt_jan_code');
	    nlapiSetCurrentLineItemValue('recmachcustrecord_djkk_pldt_pl_fd', 'custrecord_djkk_price_subsidiary_pl_fd', sub, false, true);
	    
        // DJ_ブランド
        if (dtBrand) {
            nlapiSetCurrentLineItemValue('recmachcustrecord_djkk_pldt_pl_fd', 'custrecord_djkk_pldt_brand', dtBrand, false, true);
        }
        // DJ_商品コード
        if (dtItemCode) {
            nlapiSetCurrentLineItemValue('recmachcustrecord_djkk_pldt_pl_fd', 'custrecord_djkk_pldt_itemcode_fd', dtItemCode, false, true);
        }
        // DJ_仕入先
        if (dtSupplier) {
            nlapiSetCurrentLineItemValue('recmachcustrecord_djkk_pldt_pl_fd', 'custrecord_djkk_pldt_supplier_fd', dtSupplier, true, false);
        }
        // DJ_商品名(日本語)
        if (dtItemNameJp) {
            nlapiSetCurrentLineItemValue('recmachcustrecord_djkk_pldt_pl_fd', 'custrecord_djkk_pldt_itemname_jp_fd', dtItemNameJp, true, false);
        }
        // DJ_商品名(英語)
        if (dtItemnameEn) {
            nlapiSetCurrentLineItemValue('recmachcustrecord_djkk_pldt_pl_fd', 'custrecord_djkk_pldt_itemname_en_fd', dtItemnameEn, true, false);
        }
        // DJ_カタログコード
        if (dtCatalogCode) {
            nlapiSetCurrentLineItemValue('recmachcustrecord_djkk_pldt_pl_fd', 'custrecord_djkk_pldt_catalog_code', dtCatalogCode, true, false);
        }
        // DJ_JANコード
        if (dtJanCode) {
            nlapiSetCurrentLineItemValue('recmachcustrecord_djkk_pldt_pl_fd', 'custrecord_djkk_pldt_jan_code', dtJanCode, true, false);
        }
	} else {
	    if(dtSub != sub){
	        nlapiSetCurrentLineItemValue('recmachcustrecord_djkk_pldt_pl_fd', 'custrecord_djkk_price_subsidiary_pl_fd', sub, false, true);
			...
			if(i+1 == lineNum){
				...
				if(nlapiGetLineItemValue(listType, 'custrecord_djkk_pldt_itemcode_fd', i+1) == itemCd){
				        ...
				        if (lineSaleprice != saleprice) {
				            ...
				            if (idiFlg) {
				                alert('有効期間が重複しているので、ご確認お願いいたします。');
				                return false;
				            }
				        } else {
