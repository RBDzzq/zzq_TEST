...
suitel10n.jp.comp.su = suitel10n.jp.comp.su || {};

suitel10n.jp.comp.su.SalesPurchaseReport = function SalesPurchaseReport() {
     ...
     *}
     */
    this.doGet = function (request){
        var requestObj = {
                subsidiary: request.getParameter("subsidiary"),
                periodStart: request.getParameter("periodStart"),
    ...
    function initializeTaxCodeClassObj(){
        var taxCodeClassObj = {};
        var taxCodeSearch = new ns_wrapper.Search(TAX_CODE_RECORD);
        taxCodeSearch.addColumn(TAX_CLASSIFICATION);
        taxCodeSearch.addColumn(NAME);
        taxCodeSearch.addFilter(COUNTRY,IS_OPERATOR,JAPAN,'');
        var iter = taxCodeSearch.getIterator();
        while(iter.hasNext()){
            var result = iter.next();
            var taxClass = result.getValue(TAX_CLASSIFICATION)
            if(taxClass){
                taxCodeClassObj[result.getValue(NAME)] = taxClass;
            }
        }
    ...
    function checkValidParams(params){
        ...
        if (isOneWorld() && !params.subsidiary) {
...
suitel10n.jp.comp.su.run = function(request, response) {
	nlapiLogExecution('AUDIT', 'request parameters', JSON.stringify(request.getAllParameters()));
    var rsSalesPurchaseReport = new suitel10n.jp.comp.su.SalesPurchaseReport();
    response.write(rsSalesPurchaseReport.doGet(request));
}