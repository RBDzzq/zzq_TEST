 ...
 */
define(['N/search', 'N/record', 'N/runtime', 'N/format', 'N/config', 'me', 'moment', 'underscore', 'N/error','N/email'],
    function(search, record, runtime, format, config, me, moment, _, error, email) {
        ...
        function reduce(context) {
            try {
                ...
                var headRecord = record.load({
                    type: 'customrecord_custpayment_management',
                    id: id,
                    isDynamic: true
                });
        ...
        function doGetInvoiceFromClient(client) {
            var invoiceDetailsList = getInvoiceList();
            // var searchResults = invoiceDetailsList.getRange({
            //     start: 0,
            //     end: 1000
            ...
            if (invoiceDetailsList != null) {
                ...
                do {
                    searchlinesResults = invoiceDetailsList.getRange({
                        start: searchCount,
            ...
            var invoiceId = null;
            for (var n = 0; n < searchCustResults.length; n++) {

                var entityName = searchCustResults[n].getText({name: 'entity'});
                entityName = entityName.substring(entityName.indexOf(" ")+1);
                log.debug("doGetInvoiceFromClient searchResults[n].getValue({name: 'entity'}):",entityName);
                if ((entityName+" ").indexOf(" "+client+" ") > -1) {
                    invoiceId = searchCustResults[n].id;
                    break;
                }
            }
            return invoiceId;
        ...
        function doCreateCustomerPaymentWithSaving(headRecord, accountId, saving, options, lineNum, clientValue) {
        ...
        function doCreateCustomerPayment(headRecord, accountId, options) {
                ...
                for (var lineNum = 0; lineNum < numLines; lineNum++) {
                    try {
                        ...
                        log.debug('customerLookup.subsidiary',customerLookup.subsidiary[0].value)

                        var myCustomListSearch = search.create({
                            type : 'customrecord_custpayment_sub_acc',
                            filters : [["custrecord_dj_subsidiary", "is", customerLookup.subsidiary[0].value], ],
                            columns : [ {
                        ...
                        if (results && results.length > 0) {
                            ...
                            log.debug('customrecord_custpayment_sub_acc accountId',accountId);
                        }
                        if (!accountId) {

                            log.debug('subsidiary:'+customerLookup.subsidiary[0].value,'この子会社の勘定科目が存在しない。')
                            continue;
                        }
                        ...
                        if (me.isEmpty(saving))
                            doCreateCustomerPaymentFromCSV(headRecord, accountId, lineNum,client);
                        else {
                            ...
                            if (_.isEmpty(savingObj.invoice))
                                ...
                                doCreateCustomerPaymentWithSaving(headRecord, accountId, savingObj, options, lineNum,client);
        ...
        function doCreateCustomerPaymentFromCSV(headRecord, accountId, i,clientValue) {
        ...
        function getSetting() {
            try {
                var mysearch = search.create({
                    type: 'customrecord_custpayment_setting',
                    columns: [
